...
void Params::init(const std::unordered_map<std::string, std::string>& config) {
  for (auto iter = init_optional_params_.begin(); iter != init_optional_params_.end(); ++iter) {
  ...
  for (const auto& req : init_required_params_) {
    ...
    if (iter_config == config.end()) {
      ...
      throw std::invalid_argument("Params: Incomplete configuration: missing required parameter");
    } else {
...
std::string& Params::at(const std::string& key) {
  ...
  HAMI_ASSERT(iter != config_.end(), "Params: key not found: " + key);
  return iter->second;
}
