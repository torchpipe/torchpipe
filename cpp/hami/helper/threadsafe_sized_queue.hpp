...
#include <thread>
#include <vector>
#include "hami/helper/sized_queue.hpp"
...
class ThreadSafeSizedQueue {
 public:
  ...
  void push(const std::vector<T>& new_value, const std::vector<size_t>& sizes) {
    std::lock_guard<std::mutex> lk(mut_);
    for (size_t i = 0; i < new_value.size(); ++i) data_queue_.push(new_value[i], sizes[i]);

    data_cond_.notify_all();
  }

  void push(const std::vector<T>& new_value, const std::function<int(const T&)>& req_size_func) {
    std::lock_guard<std::mutex> lk(mut_);
  ...
  bool WaitForPopWithSize(T& value, int time_out, std::function<bool(std::size_t)> check) {
    std::unique_lock<std::mutex> lk(mut_);
    auto re = data_cond_.wait_for(lk, std::chrono::milliseconds(time_out), [this, check] {
