...
class ThreadSafeQueue {
   ...
   public:
    ...
    template <typename Rep, typename Period, typename U,
              typename = std::enable_if_t<std::is_convertible_v<U, T>>>
    bool try_put(U &&value, size_t maxSize,
    ...
    bool try_put(const Container<T> &values, size_t maxSize,
                 std::chrono::duration<Rep, Period> timeout) {
        {
    ...
    std::optional<T> try_get(std::chrono::duration<Rep, Period> timeout) {
        ...
        if (!pushed_cond_.wait_for(lock, timeout,
                                   [this] { return !queue_.empty(); })) {
            return std::nullopt;
        }
...
class ThreadSafeSizedQueue {
   ...
   public:
    ...
    bool wait_until_at_most(size_t max_size,
                            std::chrono::duration<Rep, Period> timeout) {
        std::unique_lock<std::mutex> lock(mutex_);
        return popped_cond_.wait_for(lock, timeout, [this, max_size] {
    ...
    bool wait_until_at_least(size_t min_size,
                             std::chrono::duration<Rep, Period> timeout) {
        std::unique_lock<std::mutex> lock(mutex_);
        return pushed_cond_.wait_for(lock, timeout, [this, min_size] {
