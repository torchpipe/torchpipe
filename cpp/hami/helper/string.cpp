...
std::vector<std::string> items_split(std::string strtem, char a, char left, char right) {
  std::vector<std::string> strvec;
...
std::vector<std::string> flatten_brackets(const std::string& strtem_, char left, char right) {
  ...
  while (!strtem.empty()) {
    ...
    if (iter_begin != std::string::npos) {
      ...
      HAMI_ASSERT(iter_end == strtem.size() - 1, "brackets not match: " + strtem_);
...
std::string brackets_split(const std::string& strtem_,
                           std::unordered_map<std::string, std::string>& config, char left,
                           char right) {
