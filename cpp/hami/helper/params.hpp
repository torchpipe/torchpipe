...
/**
 * @brief Manages parameters for each Backend. It maintains two types of parameters:
...
class Params {
 public:
  ...
  Params(std::unordered_map<std::string, std::string> init_optional_params,
   ...
   * - Saves optional parameters (init_optional_params) to the internal parameter _config
   *   (which aggregates all required parameters and their values). If the input parameter
   *   config has updated values, it saves the updated values to _config.
   * - Checks if config contains all required parameters (init_required_params) and saves
   *   them to _config. If not, it prints an error and returns false, indicating initialization
   * failure.
   *
