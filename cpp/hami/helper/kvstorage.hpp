...
template <typename GroupKey = std::string, typename EntryKey = std::string,
...
class ConcurrentKVStore {
   ...
   public:
     ...
     */
    void insert_or_assign(const GroupKey& group_id, const EntryKey& key,
                          const Value& value) {
     ...
     */
    bool try_insert_or_assign(const GroupKey& group_id, const EntryKey& key,
                              const Value& value) {
        std::shared_lock read_lock(store_mutex_);
        auto group_it = store_.find(group_id);
        if (group_it == store_.end()) {
     ...
     */
    void upsert(const GroupKey& group_id, const EntryKey& key,
                const Value& value) {
    ...
    void add_group(const GroupKey& group_id) {
        ...
        auto group_it = store_.find(group_id);
        if (group_it != store_.end()) {
            throw std::runtime_error("Request group already exists: " +
                                     group_id);
        }
     ...
     * return false if group or entry not exist
     */
    bool try_contains_entry(const GroupKey& group_id,
                            const EntryKey& key) const {
        std::shared_lock read_lock(store_mutex_);
        auto group_it = store_.find(group_id);
        if (group_it == store_.end()) {
