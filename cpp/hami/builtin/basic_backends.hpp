 ...
 *
 * This backend allows injecting a dependency, which can be used for chaining or delegation.
 */
class HAMI_EXPORT Dependency : public Backend {
 public:
  /**
   * @brief Injects a dependency into the backend or the backend's dependency if alread exists.
   *
  ...
  virtual void pre_init(const std::unordered_map<std::string, std::string>& config,
 ...
 private:
  virtual void forward_impl(const std::vector<dict>& input_output, Backend* dependency);
 ...
 * As it has multiple dependent backends, we treat it as a `container` of backends and it is not
 * allowed to modify its dependencies at runtime.
 * @note This class supports the syntax `Container[(pre=1)B(post=z,post2=z2)[Z],C]`.
 */
class Container : public Backend {
 public:
  /**
   * @param *::dependency The name of the sub-backends, multiple backends separated by commas.
   * @remark
   * 1. The initialization of sub-backends will be executed in **reverse order**.
   * 2. This container supports expanding bracket compound syntax. It uses the @ref
   * str::brackets_split function to expand it as follows:
   * - backend = B[C]        =>     {backend=B, B::dependency=C}
   * - backend = D           =>     {backend=D}
   * - backend = B[E[Z1,Z2]] =>     {backend=B, B::dependency=E[Z1,Z2]}
   * - It supports prefix and suffix configuration syntax, i.e., (pre=1)A(post=z)[B].
   *   The prefix configuration is stored in the protected member variable base_config_,
   *   and the suffix configuration is passed to A.init.
   */
  void init(const std::unordered_map<std::string, std::string>& config,
            const dict& dict_config) override final;
  virtual void post_init(const std::unordered_map<std::string, std::string>& config,
...
/**
 * @brief A specialized backend that supports only single input and no dependency.
 *
 * Enforces the constraint that only one input is allowed and no dependency can be provided.
 */
 ...
 public:
  ...
  void forward(const std::vector<dict>& input_output) override final {
    if (input_output.size() != 1) {
      throw std::invalid_argument("SingleBackend only supports single input");
    }
   ...
   * @throws std::runtime_error Always, as SingleBackend does not support dependencies.
   */
  void inject_dependency(Backend* dependency) override final {
    throw std::runtime_error("SingleBackend does not support inject dependency");
  }
};
...
class HAMI_EXPORT List : public Backend {
 public:
  ...
  void forward(const std::vector<dict>& input_output) override final;
  [[nodiscard]] size_t max() const override final { return 1; }
  [[nodiscard]] size_t min() const override final { return std::numeric_limits<size_t>::max(); }
