...
class Random : public Backend {
 private:
  void impl_init(
    ...
    HAMI_ASSERT(deps.size() == 2, "error dep: " + dep);

    dependency_a_ = HAMI_INSTANCE_GET(Backend, deps[0]);
    dependency_b_ = HAMI_INSTANCE_GET(Backend, deps[1]);
    HAMI_ASSERT(dependency_a_ && dependency_b_, "Dependency not found: " + dep);
    SPDLOG_INFO(
    ...
    HAMI_ASSERT(dependency_a_->max() == dependency_b_->max());
    HAMI_ASSERT(
