...
namespace hami {
void Dependency::init(const std::unordered_map<std::string, std::string>& config,
  ...
  if (!dependency_name_.empty()) {
    auto backend = std::shared_ptr<Backend>(HAMI_CREATE(Backend, dependency_name_));
    HAMI_ASSERT(backend, "`" + dependency_name_ + "` is not a valid backend");
...
void Dependency::inject_dependency(Backend* dependency) {
  ...
  if (injected_dependency_) {
    thread_local const auto log_tmp = []() {
      SPDLOG_WARN(
          "Dependency::inject_dependency: dependency already exists(may happened in the "
...
void Dependency::forward_impl(const std::vector<dict>& input_output, Backend* dependency) {
...
void Container::init(const std::unordered_map<std::string, std::string>& config,
  ...
  HAMI_ASSERT(iter != config.end(),
              "Dependency configuration " + *name +
                  ...
                  "Containers do not allow runtime dynamic modification of dependencies, "
  ...
  if (!backend_setting.empty()) {
    ...
    HAMI_ASSERT(backend_names.size() >= 1, "Container: backend_names.size() should >= 1");
    ...
    for (std::size_t i = 0; i < backend_names.size(); ++i) {
      ...
      auto backend = str::prefix_parentheses_split(engine_name, prefix_str);  // (params1=a)A
      ...
      if (!post_str.empty()) {
        ...
        SPDLOG_INFO("backend : {} pre: `{}` post: `size={}`", engine_name, prefix_str,
                    new_config.size());
      }
      auto main_backend = str::brackets_split(backend, new_config);
      // HAMI_ASSERT(new_config.find("backend") != new_config.end());
      if (pre_config.find("backend") == pre_config.end()) pre_config["backend"] = main_backend;
      base_config_.push_back(pre_config);
      auto backend_ptr = std::unique_ptr<Backend>(HAMI_CREATE(Backend, main_backend));
...
std::pair<size_t, size_t> Container::update_min_max(const std::vector<Backend*>& depends) {
