...
class DagParser {
 public:
  ...
  // const std::unordered_set<std::string>& get_previous(const std::string& node) {
  //   return dag_config_.at(node).previous;
  // }

  bool skip_if_no_result(const std::string& node) { return dag_config_.at(node).or_filter; }

  dict prepare_data_from_previous(const std::string& node,
