...
namespace hami
{
        ...
        const dict &in_kwargs)
    {
        ...
        for (size_t i = 0; i < base_config_.size(); ++i)
        {
            ...
            while (phases[1].find(main_backend, find_start) != std::string::npos)
            {
                    ...
                    reg_backends.end())
                {
                    ...
                    reg_backends[base_dependencies_[i].get()] =
                        register_name;
                }
                else
                {
        ...
        HAMI_ASSERT(forward_backend_, "IoCV0 init failed");
        ...
        SPDLOG_INFO("IoCV0, forward phase: {}, [{}, {}]", phases[1],
                    forward_backend_->min(), forward_backend_->max());
    }

    void IoCV0::init_phase(const std::string &phase_config,
    ...
    class IoC : public ControlPlane
    {
            ...
            const dict &options) override
        {
            ...
            size_t num_semicolon = std::count(delimiters_.begin(), delimiters_.end(), ';');
            HAMI_ASSERT((num_semicolon == 1 && delimiters_.back() == ';'));

            // initialization phase
            for (size_t i = 0; i < backend_cfgs_.size() - 1; ++i)
            {
                std::unique_ptr<Backend> backend = init_backend(backend_cfgs_[i], params, options);
                backends_[i] = std::move(backend);
            }

            // forward phase
            std::unordered_map<void *, std::string> reg_backends;
            std::unordered_set<std::string> main_backends_set(main_backends_.begin(), main_backends_.end());
            ...
            for (size_t i = 0; i < main_backends_.size() - 1; ++i)
            {
                ...
                size_t find_start = 0;
                std::unordered_map<void *, std::string> reg_backends;
                while (forward_backend.find(main_backend, find_start) != std::string::npos)
                {
                    std::string register_name;
            ...
            std::unique_ptr<Backend> backend = init_backend(forward_backend, params, options);
            backends_.push_back(std::move(backend));
        }

        void impl_forward(const std::vector<dict> &io) { return backends_.back()->forward(io); }
        [[nodiscard]] virtual size_t impl_min() const override { return backends_.back()->min(); }
