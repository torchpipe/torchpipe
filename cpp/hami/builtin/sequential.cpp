...
namespace hami
{
    ...
    void SequentialV0::impl_forward(const std::vector<dict> &io)
    {
        ...
        for (std::size_t i = 0; i < base_dependencies_.size(); ++i)
        {
            ...
            else
            {
                ...
                for (std::size_t j = 0;
                     j < io.size() && break_index.count(j) == 0; ++j)
                {
    ...
    HAMI_REGISTER(Backend, SequentialV0, "SequentialV0, S_v0");

    void Sequential::impl_custom_init(const std::unordered_map<std::string, std::string> &params,
                                      const dict &options)
    {
        ...
        for (size_t i = 0; i < backend_cfgs_.size(); ++i)
        {
            ...
            std::unique_ptr<Backend> backend = init_backend(backend_cfgs_[ri], params, options);
        ...
        for (std::size_t i = 0; i < backends_.size(); ++i)
        {
            ...
            else
            {
                     ...
                     j < io.size() && break_index.count(j) == 0; ++j)
                {
                    ...
                    {
                        dicts_guard.erase(TASK_RESULT_KEY);
                        throw std::runtime_error("Sequential: no result in " + backend_cfgs_[i]);
                        break_index.insert(j);
                        if (break_index.size() == io.size())
                            return;
                    }
