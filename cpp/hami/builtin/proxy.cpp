...
void BackendProxy::init(const std::unordered_map<string, string>& config, const dict& dict_config) {
  auto iter = config.find("backend");
  HAMI_ASSERT(iter != config.end(),
              "BackendProxy configuration error: 'backend' parameter is missing. Please ensure it "
  ...
  owned_backend_ = std::unique_ptr<Backend>(HAMI_CREATE(Backend, main_backend));
