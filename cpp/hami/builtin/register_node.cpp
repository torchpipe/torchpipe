...
namespace hami {
void RegisterNode::pre_init(const std::unordered_map<std::string, std::string>& config,
...
HAMI_REGISTER(Backend, RegisterNode, "RegisterNode,Node");

class RegisterInstances : public Backend {
 public:
  ...
  void init(const std::unordered_map<std::string, std::string>& config,
    ...
    constexpr auto default_name = "RegisterInstances";
    ...
    if (iter != config.end()) {
      ...
      throw std::runtime_error("Dependency configuration  " + *name +
                               "::dependency not found, skipping dependency injection process");
    }
    ...
    if (iter == config.end()) {
      ...
      auto [ptr, ec] = std::from_chars(iter->second.data(),
                                       iter->second.data() + iter->second.size(), instance_num);
    ...
    for (size_t i = 0; i < instance_num; ++i) {
      owned_backend_ = std::shared_ptr<Backend>(HAMI_CREATE(Backend, dependency_name));
      HAMI_ASSERT(owned_backend_, "`" + iter->second + "` is not a valid backend");
      sub_config[TASK_INDEX_KEY] = std::to_string(i);
      owned_backend_->init(sub_config, sub_dict_config);

      HAMI_INSTANCE_REGISTER(Backend, node_name + "." + std::to_string(i), owned_backend_);
    }
  }

  void forward(const std::vector<dict>& inputs) override final { owned_backend_->forward(inputs); }
};

HAMI_REGISTER(Backend, RegisterInstances, "RegisterInstances,Instances");
