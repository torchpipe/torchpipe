...
size_t count(const str::mapmap& config) {
  return config.find(TASK_GLOBAL_KEY) == config.end() ? config.size() : config.size() - 1;
}
...
DagParser::DagParser(const str::mapmap& config) {
  ...
  // // if a node has been cited by multiple nodes(with next/map), it's data must be acquired by
  // deep copy
      ...
      for (const auto& cited_item : item.second.cited) {
        if (dag_config_[cited_item].map_config.empty()) {
          SPDLOG_INFO("DagParser: node `" + item.first +
                      "` has been cited(through map or next) by more than one node, but one "
                      "of them - " +
  ...
  for (const auto& item : dag_config_) {
    if (item.second.cited.size() > 1) {
      HAMI_ASSERT(!item.second.or_filter, "DagParser: `map` and `or` cannot be used together");
    }
  }
...
void DagParser::update_previous() {
  for (auto& item : dag_config_) {
    for (const auto& next : item.second.next) {
      ...
      HAMI_ASSERT(iter != dag_config_.end(), "DagParser: next node not found: " + next);
      // item.first => next
      iter->second.previous.insert(item.first);
    }
    if (item.second.map_config.empty()) {
      HAMI_ASSERT(item.second.previous.size() <= 1, "DagParser: " + item.first +
                                                        " has more than "
...
str::mapmap DagParser::parse_map_config(const std::string& config) {
  ...
  for (const auto& maps_item : maps) {
    ...
    HAMI_ASSERT(maps_data.size() == 2, "map should be in the form of node_name[src_key:dst_key]");
...
std::unordered_set<std::string> DagParser::get_subgraph(const std::string& root) {
  // std::unordered_set<std::string> re;
  HAMI_ASSERT(dag_config_.find(root) != dag_config_.end(), "DagParser: " + root + " not found");
...
dict DagParser::prepare_data_from_previous(const std::string& node,
  ...
  if (dag_config_.at(node).map_config.empty()) {
    ...
    if (re->find(TASK_RESULT_KEY) == re->end()) {
      if (!dag_config_.at(node).or_filter) {
        throw std::runtime_error("DagParser: " + previous_node + " has no result");
      }
    } else {
      if (dag_config_.at(node).or_filter) {
        processed[node] = re;
  ...
  for (const auto& item : dag_config_.at(node).map_config) {
    ...
    for (const auto& [src_key, dst_key] : item.second) {
      ...
      HAMI_ASSERT(iter != src_dict->end(), "DagParser: " + src_key + " not found in " + src_node);
  ...
  HAMI_ASSERT(re->find(TASK_DATA_KEY) != re->end(),
