...
void Interpreter::init(const std::unordered_map<std::string, std::string>& config,
  ...
  if (iter == new_dict_config->end()) {
    (*new_dict_config)[TASK_CONFIG_KEY] = str::mapmap{{TASK_GLOBAL_KEY, config}};
  ...
  HAMI_FATAL_ASSERT(node_names.size() > 0, "Interpreter: no node found in config");
  ...
  for (const auto& item : dual_config) {
    ...
    if (iter_init != item.second.end()) {
      ...
      SPDLOG_INFO("Interpreter: `init` not found in node config, using default: {}",
                  DEFAULT_INIT_CONFIG);
  ...
  if (iter_entry != global_config.end()) {
    ...
    entry_name = parser::count(dual_config) == 1 ? "Forward[node." + *node_names.begin() + "]"
                                                 : "DagDispatcher";
    SPDLOG_INFO("Interpreter: `entrypoint` not found in global config, using default: {}",
                entry_name);
  }
