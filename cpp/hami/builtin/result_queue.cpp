...
namespace hami
{
    ...
    {
        while (!queue_->try_put(input, 1, queue_max_,
    ...
    class Send2Queue : public Backend
    {
    private:
                       ...
                       const dict &) override final
        {
            ...
            {
                HAMI_ASSERT(false, "Send2Queue must have at most one queue instance name");
            }
    ...
    private:
        ...
        void impl_forward(const std::vector<dict> &input) override
        {
            while (!owned_queue_->try_put(input, 1, queue_max_,
                                          std::chrono::milliseconds(SHUTDOWN_TIMEOUT)))
            {
            };
        }
    ...
    void
    Observer::impl_forward(const std::vector<dict> &input)
    {
        for (auto &item : input)
        {
