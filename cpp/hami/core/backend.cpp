...
namespace hami
{
    ...
    //     std::unordered_map<std::string, std::string> dst_config, const dict &kwargs,
    //     const std::string &aspect_name_str)
    // {
    ...
    std::unique_ptr<Backend> init_backend(
        const std::string &backend_config,
        std::unordered_map<std::string, std::string> dst_config, const dict &dict_kwargs,
        const std::string &aspect_name_str)
    {
    ...
    HAMI_REGISTER_BACKEND(BackendMax);
...
namespace hami::meta
{
    ...
    {
        auto name = HAMI_OBJECT_NAME(Backend, obj_ptr);
        if (name)
        {
            cls_name = *name;
            return true;
    ...
    get_args_kwargs(const Backend *obj_ptr, std::string cls_name, const std::unordered_map<std::string, std::string> &config)
    {
        auto name = HAMI_OBJECT_NAME(Backend, obj_ptr);
        if (name)
        {
        ...
        {
            HAMI_ASSERT(!cls_name.empty(), "This instance was not created via reflection");
        }
