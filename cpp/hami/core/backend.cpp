...
std::unique_ptr<Backend> create_backend(const std::string& class_name,
  ...
  auto backend = std::unique_ptr<Backend>(HAMI_CREATE(Backend, class_name, aspect_name_str));
  HAMI_ASSERT(backend != nullptr, "Failed to create backend " + class_name + " through reflection");
  return backend;
};

void register_backend(const std::string& aspect_name_str, std::shared_ptr<Backend> backend) {
...
std::unique_ptr<Backend> init_backend(const std::string& backend_config,
  ...
  auto backend = std::unique_ptr<Backend>(HAMI_CREATE(Backend, main_backend, aspect_name_str));
  HAMI_ASSERT(backend != nullptr,
