...
static inline dict make_dict() { return std::make_shared<std::unordered_map<string, hami::any>>(); }
...
extern void throw_not_exist(const string& key);
...
struct TypedDict {
  using BaseType =
      std::variant<bool, int, std::shared_ptr<TypedDict>, string, double,
