...
namespace hami::parser_v2
{
    ...
    struct BracketPair
    {
        ...
        BracketPair(char l, char r) : left(l), right(r)
        {
            if (l == r)
            {
                throw std::invalid_argument("Bracket pairs cannot have identical left/right characters");
            }
        }
    };
     ...
     * @throws std::invalid_argument for mismatched brackets or invalid delimiter positions
     */
    ...
    /**
     * @brief Parses the input string and extracts specific patterns based on bracket pairs.
     *
     * @param input The input string to be parsed.
     * @param open_to_close_bracket A map of opening brackets to their corresponding closing brackets.
     * @return std::vector<std::pair<std::string, char>> A vector of pairs containing the extracted strings and corresponding characters.
     *
     * Example usage:
     * - Input: "a[b(d)]"
     *   Output: [ {"a", ''}, {"b(d)", '['} ]
     ...
     *   Output: [ {"A", '(xx,de3)B'} ]
     */
    std::vector<std::pair<std::string, char>> expend_outmost_brackets(const std::string &input, std::unordered_map<char, char> open_to_close_bracket = {{'(', ')'}, {'{', '}'}, {'[', ']'}});

    /**
     * @brief Recursively extracts nested bracket contents while preserving delimiter-separable blocks
     *
     ...
     *
     * @param strtem_in Input string to process (assumed bracket-balanced and space-cleaned)
     ...
     *
     * @note Preconditions:
     * - Input must be pre-validated with are_brackets_balanced() and should not be delimiter separable.
    ...
    public:
        explicit Parser(
            std::vector<BracketPair> bracket_pairs = {
                {'(', ')'}, {'[', ']'}, {'{', '}'}, {'<', '>'}},
            std::unordered_set<char> delimiters = {',', ';'}) : bracket_pairs_(std::move(bracket_pairs)), delimiters_(std::move(delimiters))
        {
            build_bracket_maps();
        }
        ...
        parse_args_kwargs(std::string config)
        {
            return hami::parser::parse_args_kwargs(config);
        }
