...
namespace hami::parser_v2
{
        ...
        const std::unordered_set<char> &delimiters)
    {
        ...
        for (const auto &pair : bracket_pairs)
        {
            ...
            {
                SPDLOG_ERROR("Duplicate bracket definition: {}-{}", pair.left, pair.right);
        ...
        for (size_t pos = 0; pos < input.size(); ++pos)
        {
            ...
            else if (delimiters.count(c))
            {
                ...
                { // Redundant check for safety
                    SPDLOG_ERROR("Invalid delimiter '{}' at string boundary in: {}", c, input);
    ...
    /**
     * @brief Determines if an input string can be split by delimiters outside bracket-enclosed regions.
     *
     * This function splits the input string into tokens separated by specified delimiters, while treating content
     * within bracket pairs as indivisible blocks. Bracket nesting is fully supported. The function populates
     * two vectors: one containing the extracted delimiters in order, and another containing the resulting tokens.
     *
     * @param input The string to be processed. Expected to have balanced brackets and spaces/control characters removed(not verified internally).
     * @param bracket_pairs Bracket pairs to consider as block boundaries. Default: { '()', '{}', '[]' }.
     * @param delimiters Characters to use as token separators outside brackets. Default: { ',', ';' }.
     * @param[out] result_delimiters Receives encountered delimiters in order. Size will be tokens.size()-1.
     * @param[out] result_output Receives parsed tokens with result_delimiters.size() + 1 == result_output.size() always hold.
     * @return true If at least one valid delimiter was found and splitting occurred.
     * @return false If no delimiters were found in non-bracketed regions.
     *
     * @note
     * - Empty tokens may result from consecutive delimiters or leading/trailing delimiters
        ...
        const std::unordered_set<char> &delimiters = {',', ';'})
    {
        ...
        HAMI_FATAL_ASSERT(result_delimiters.size() + 1 == result_output.size(), "size not match");
        ...
        const std::unordered_map<char, char> open_to_close = {{'(', ')'}, {'{', '}'}, {'[', ']'}})
    {
        ...
        for (const auto &pair : open_to_close)
        {
            ...
            {
                throw std::invalid_argument("Duplicate bracket characters in pair definitions");
            }
    ...
    std::vector<std::pair<std::string, char>> flatten_brackets(const std::string &strtem_in)
    {

        std::vector<std::pair<std::string, char>> re = expend_outmost_brackets(strtem_in);
        HAMI_ASSERT(re.size() >= 1 && re.size() <= 3 && re[0].second == 0, "illegal input: " + strtem_in + ". Support A(args, kwsrag)[B]");
        ...
        if (re.size() == 2)
        {
            HAMI_ASSERT(re[1].second == '(' || re[1].second == '[', "illegal input: " + strtem_in);
        }
        if (re.size() == 3)
        {
            HAMI_ASSERT(re[1].second == '(' && re[2].second == '[', "illegal input: " + strtem_in);
        }
        if (re.back().second == '[')
        {
            HAMI_ASSERT(!re.back().first.empty());
            if (is_delimiter_separable((re.back().first)) || re.back().first[0] == '(')
            {
                return re;
            }
    ...
    std::pair<std::string, std::string> Parser::prifix_split(const std::string &input, char left_bracket, char right_bracket)
    {
        HAMI_ASSERT(!input.empty() && input[0] == left_bracket);
        std::stack<char> bracket_stack;
    ...
    std::string
    Parser::parse(const std::string &input, std::unordered_map<std::string, std::string> &config_output)
    {
        ...
        for (size_t i = 1; i < result.size(); i++)
        {
            if (result[i].second == '(')
            {
                ...
                HAMI_ASSERT(!dep.empty());
                // if (dep[0] == '(')
                // {
                //     dep = remove_bracket(dep, ')');
                // }
                auto insert_re = config_output.insert({dep + "::args", result[i].first});
                HAMI_ASSERT(insert_re.second, "Duplicate args key: " + dep + "::args"); // result[i - 1].second == 0 &&
                args_index.insert(i);
            }
        }
        ...
        for (size_t i = 1; i < new_brackets.size(); i++)
        {
            ...
            auto insert_re = config_output.insert({new_brackets[i - 1].first + "::dependency", new_brackets[i].first});
            HAMI_ASSERT(insert_re.second, "Duplicate dependency key: " + new_brackets[i - 1].first + "::dependency");
        }

        return result[0].first;
    }

    std::vector<std::string> Parser::split_by_delimiter(const std::string &input, char delimiter)
    {

        std::vector<char> result_delimiters;
        std::vector<std::string> result_output;
        HAMI_ASSERT(is_delimiter_separable(
            input,
            result_delimiters,
            result_output, {{'(', ')'}, {'{', '}'}, {'[', ']'}}, {delimiter}));
        return result_output;
    }

    std::vector<std::pair<std::string, char>>
    expend_outmost_brackets(const std::string &input, std::unordered_map<char, char> open_to_close_bracket)
    {
        ...
        for (size_t i = 0; i < input.size();)
        {
            ...
            {
                HAMI_ASSERT(!bracket_stack.empty() && open_to_close_bracket[bracket_stack.top()] == c,
                            "Mismatched brackets");
        ...
        HAMI_ASSERT(bracket_pos.size() == 2 * bracket_type.size() && !bracket_pos.empty());
        std::vector<std::pair<std::string, char>> result;

        for (size_t i = 0; i < bracket_type.size(); ++i)
        {
            if (bracket_type[i] == 0)
                result.emplace_back(input.substr(bracket_pos[2 * i], bracket_pos[2 * i + 1] - bracket_pos[2 * i] + 1), bracket_type[i]);
            else
            {
                result.emplace_back(input.substr(bracket_pos[2 * i] + 1, bracket_pos[2 * i + 1] - bracket_pos[2 * i] - 1), bracket_type[i]);
            }
        }
        ...
        {
            HAMI_ASSERT(result.size() > 0 && result[0].second == '(' && result[1].second == 0);
            result[1].first = result[0].second + result[0].first + open_to_close_bracket[result[0].second] + result[1].first;
...
namespace hami
{
    class ParserTest : public BackendOne
    {
        ...
        void forward(const dict &data) override
        {
            ...
            HAMI_ASSERT(!config.empty());
            hami::parser_v2::Parser parser;

            std::unordered_map<std::string, std::string> config_output;
            if (hami::parser_v2::is_delimiter_separable(config) || config[0] == '(')
            {
            ...
            else
            {
                ...
                data->insert_or_assign(TASK_RESULT_KEY, std::make_pair(main_bkd, config_output));
            }
        }
    };
