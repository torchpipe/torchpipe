...
namespace hami::parser_v2
{
    bool has_valid_unnested_delimiters(
        const std::string &input, const std::vector<BracketPair> &bracket_pairs,
        const std::unordered_set<char> &delimiters)
    {
        ...
        for (const auto &pair : bracket_pairs)
        {
            ...
            {
                SPDLOG_ERROR("Duplicate bracket definition: {}-{}", pair.left,
                             pair.right);
        ...
        for (size_t pos = 0; pos < input.size(); ++pos)
        {
            ...
            else if (valid_right_brackets.count(c))
            {
                if (bracket_stack.empty() ||
                    left_to_right[bracket_stack.top()] != c)
                {
                    SPDLOG_ERROR("Mismatched '{}' at position {} in: {}", c, pos,
                                 input);
            ...
            else if (delimiters.count(c))
            {
                ...
                { // Redundant check for safety
                    SPDLOG_ERROR("Invalid delimiter '{}' at string boundary in: {}",
                                 c, input);
        ...
        if (!bracket_stack.empty())
        {
            SPDLOG_ERROR("Unclosed '{}' bracket in: {}", bracket_stack.top(),
                         input);
    ...
    bool is_delimiter_separable(
        const std::string &input, std::vector<char> &result_delimiters,
    ...
    std::pair<std::string, std::string> Parser::prifix_split(
        const std::string &input, char left_bracket, char right_bracket)
    {
        HAMI_ASSERT(!input.empty());
                                                        ...
                                                        char delimiter)
    {
        ...
        std::vector<std::string> result_output;
        HAMI_ASSERT(is_delimiter_separable(input, result_delimiters, result_output,
                                           {{'(', ')'}, {'{', '}'}, {'[', ']'}},
                                           {delimiter}));
        return result_output;
    }

    std::pair<std::vector<char>, std::vector<std::string>> Parser::split_by_delimiters(const std::string &input,
        ...
        std::unordered_map<char, char> open_to_close_bracket)
    {
        ...
        if (result[0].second != 0)
        {
            ...
            result[1].first = result[0].second + result[0].first +
                              open_to_close_bracket[result[0].second] +
                              result[1].first;
...
namespace hami
{
    class ParserTest : public BackendOne
    {
        void forward(const dict &data) override
        {
            ...
            std::unordered_map<std::string, std::string> config_output;
            if (hami::parser_v2::is_delimiter_separable(config) ||
                config[0] == '(')
            {
