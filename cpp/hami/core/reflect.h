...
namespace hami
{
    ...
    public:
        ...
        std::vector<std::string> HamiGetAll()
        {
            ...
            for (auto iter = getter_map_.begin(); iter != getter_map_.end();
                 ++iter)
            {
    ...
    public:
                                  ...
                                  const std::string &aspect_name = "")
        {
            ...
            if (it == getter_map_.end())
            {
                ...
                printlog(class_name +
                         ": not registered. Registered: " + registered_cls);
        ...
        {
            {
                std::lock_guard<std::mutex> guard(class_name_map_mutex_);
                {
                    printlog("Register Named Instance(wo/ ownership): " +
                             aspect_name);
                    if (reverse_class_name_map_.find(aspect_name) !=
                        reverse_class_name_map_.end())
                    {
                        throw std::runtime_error("Duplicated backend name: " +
                                                 aspect_name);
                    }
        ...
        {
            {
                ...
                }
                {
                    ...
                    for (auto iter = reverse_class_name_map_owner_order_.rbegin(); iter != reverse_class_name_map_owner_order_.rend(); ++iter)
                    {
                        tmp_map.erase(*iter);
                    }
                }
        ...
        {
            {
                ...
                else
                {
                    ...
                    for (const auto &item : reverse_class_name_map_)
                    { // NOLINT
        ...
        // ClassMap& HamiGetMap(const std::string class_name) const { return
        // getter_map_; }
    };
    ...
    class HAMI_EXPORT ClassRegister{public : ClassRegister(
        typename ClassRegistryBase<BaseClassName>::ObjectGetter getterraw,
...
#define HAMI_INSTANCE_REGISTER(base_class_type, register_name,       \
                               backend_instance)                     \
    hami::ClassRegistryInstance<base_class_type>().DoRegisterObject( \
        register_name, backend_instance)

#define HAMI_INSTANCE_UNREGISTER(base_class_type, register_name)       \
    hami::ClassRegistryInstance<base_class_type>().DoUnRegisterObject( \
        register_name)
...
#define HAMI_ALL_NAMES(base_class_type) \
    hami::ClassRegistryInstance<base_class_type>().HamiGetAll()

/// ************************************************************************************************

#define GENERATE_BACKEND(aspect_class_type, derived_aspect_cls,            \
                         config_setting)                                   \
