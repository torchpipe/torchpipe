...
void printlog(std::string name) { SPDLOG_INFO(name); }

void print_check_distance(std::string strtem, const std::vector<std::string>& targets) {
  std::string re;
...
HAMI_EXPORT std::vector<std::string> strict_str_split(std::string strtem, char a) {
  ...
  for (auto iter_vec = strvec.begin(); iter_vec != strvec.end();) {
    // HAMI_ASSERT(!iter_vec->empty(), "src=" + strtem + ",char=" + a);
    if (iter_vec->empty())
      iter_vec = strvec.erase(iter_vec);
    else
      ++iter_vec;
  }
  HAMI_ASSERT(!strvec.empty());
  return strvec;
}

std::vector<std::array<std::string, 2>> multi_str_split(std::string strtem, char inner,
                                                        char outer) {
  auto re = std::vector<std::array<std::string, 2>>();
  auto data = strict_str_split(strtem, outer);
  for (auto& item : data) {
    auto tmp = strict_str_split(item, inner);
    HAMI_ASSERT(tmp.size() == 2);
...
ClassRegistryBase<RegistryType>& ClassRegistryInstance() {
  ...
  // SPDLOG_INFO(" class registry base: addr = {}", (long long)&(class_register));
