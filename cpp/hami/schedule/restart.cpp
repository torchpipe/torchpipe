...
void RestartEvent::pre_init(const std::unordered_map<string, string>& config,
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        threads_.emplace_back(std::thread(&RestartEvent::task_loop, this, i,
                                          task_queues_[i].get()));
    }
}
...
void RestartEvent::forward_impl(const std::vector<dict>& inputs,
                                Backend* dependency) {
...
void RestartEvent::on_start_node(dict tmp_data, std::size_t task_queue_index,
                                 Backend* dependency) {
