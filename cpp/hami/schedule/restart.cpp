...
void RestartEvent::pre_init(const std::unordered_map<string, string>& config,
  ...
  for (std::size_t i = 0; i < N; ++i) {
    ...
    threads_.emplace_back(std::thread(&RestartEvent::task_loop, this, i, task_queues_[i].get()));
  }
}

void RestartEvent::task_loop(std::size_t thread_index, ThreadSafeQueue<dict>* pqueue) {
  while (bInited_.load()) {
...
void RestartEvent::forward_impl(const std::vector<dict>& inputs, Backend* dependency) {
...
void RestartEvent::on_start_node(dict tmp_data, std::size_t task_queue_index, Backend* dependency) {
  std::shared_ptr<RestartEvent::Stack> pstack = std::make_shared<RestartEvent::Stack>();
...
void RestartEvent::on_finish_node(dict tmp_data) {
  ...
  HAMI_FATAL_ASSERT(iter != tmp_data->end());
  std::shared_ptr<Event> pre_event = any_cast<std::shared_ptr<Event>>(iter->second);
  ...
  if (pre_event->has_exception()) {
    ...
    static const std::unordered_set<std::string> ignore_keys = {TASK_STACK_KEY, TASK_EVENT_KEY,
                                                                TASK_DATA_KEY};
...
HAMI_PROXY(Restart, "Aspect[EventGuard,RestartEvent]");
...
//     auto backend_config = str::flatten_brackets("Aspect[EventGuard,RestartEvent]");
//     if (backend_config.size() != 1 && backend_config.size() != 2)
