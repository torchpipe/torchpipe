...
namespace hami
{
    ...
    void Batching::impl_inject_dependency(Backend *dependency)
    {
        ...
        {
            SPDLOG_INFO("Batching thread not inited because batching_timeout_ = 0 or max_bs = 1");
        }
    }
        ...
        while (bInited_.load())
        {
            ...
            if (cached_size == 0)
            {
                ...
                if (!input_queue_.wait_pop(
                        tmp_dict,
                        SHUTDOWN_TIMEOUT))
                { // every batching_timeout_ ms check
        ...
        const std::unordered_map<string, string> &config, const dict &dict_config)
    {
        ...
        init_task_ = [this, config, dict_config]()
        {
            ...
            HAMI_ASSERT(dependency_->min() >= 1 && dependency_->min() <= dependency_->max(), std::to_string(dependency_->min()) + " " + std::to_string(dependency_->max()));
