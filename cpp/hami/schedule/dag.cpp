...
namespace hami {
void DagDispatcher::init(const std::unordered_map<std::string, std::string>& config,
                         const dict& dict_config) {
  HAMI_ASSERT(dict_config, "DagDispatcher: dict_config is required");
  auto iter = dict_config->find(TASK_CONFIG_KEY);
  HAMI_ASSERT(iter != dict_config->end(), "DagDispatcher: config not found in dict_config");
  ...
  for (const auto& item : dual_config) {
    ...
    HAMI_ASSERT(back, "DagDispatcher: instance(" + ("node." + item.first) + ") not found");
    base_dependencies_["node." + item.first] = back;

    auto dag_backend =
        init_backend("DagProxy", {{"DagProxy::dependency", item.first}}, {}, "dag." + item.first);
  ...
  for (std::size_t i = 0; i < N; ++i) {
    ...
    threads_.emplace_back(std::thread(&DagDispatcher::task_loop, this, i, task_queues_[i].get()));
  }
}
...
void DagDispatcher::task_loop(std::size_t thread_index, ThreadSafeQueue<dict>* pqueue) {
  while (bInited_.load()) {
    dict tmp_data = nullptr;
    if (pqueue->wait_pop(tmp_data, SHUTDOWN_TIMEOUT)) {
      auto iter = tmp_data->find(TASK_STACK_KEY);
      HAMI_ASSERT(iter != tmp_data->end());
...
void DagDispatcher::on_start_node(dict tmp_data, std::size_t task_queue_index) {
  std::shared_ptr<DagDispatcher::Stack> pstack = std::make_shared<DagDispatcher::Stack>();

  pstack->task_queue_index = task_queue_index;
  pstack->input_event = any_cast<std::shared_ptr<Event>>(tmp_data->at(TASK_EVENT_KEY));
  ...
  if (iter_node_name == tmp_data->end()) {
    pstack->input_event->set_exception_and_notify_all(
        std::make_exception_ptr(std::runtime_error("DagDispatcher: `node_name` not found in input. "
...
void DagDispatcher::on_finish_node(dict tmp_data, std::shared_ptr<Stack> pstack) {
  assert(pstack);

  std::string node_name = any_cast<std::string>(tmp_data->at(TASK_NODE_NAME_KEY));

  // handle event
  auto iter = tmp_data->find(TASK_EVENT_KEY);
  if (iter != tmp_data->end()) {
    std::shared_ptr<Event> curr_event = any_cast<std::shared_ptr<Event>>(iter->second);
    ...
    if (curr_event->has_exception()) {
      if (!pstack->exception) {
        ...
        try {
          ...
          SPDLOG_WARN("Exception in node: {} has been overridden by a new exception: {}", node_name,
                      e.what());
        } catch (...) {
          SPDLOG_WARN("Exception in node: {} has been overridden by a new unknown exception",
                      node_name);
        }
      }
    }
  ...
  if (pstack->exception) {  // todo check
    if (pstack->dag.waiting_nodes.size() + pstack->dag.processed.size() == pstack->dag.total) {
      // no task is running.
...
void DagDispatcher::map_or_filter_data(std::string node_name, std::shared_ptr<Stack> pstack) {
  dict curr_data;

  try {
    curr_data = dag_parser_->prepare_data_from_previous(node_name, pstack->dag.processed);
    assert(curr_data);
...
void DagDispatcher::execute(std::string node_name, std::shared_ptr<Stack> pstack, dict curr_data) {
  auto curr_event = make_event();
  ThreadSafeQueue<dict>* local_queue = task_queues_[pstack->task_queue_index].get();
...
class DagProxy : public Backend {
 public:
  void init(const std::unordered_map<std::string, std::string>& config,
    ...
    HAMI_ASSERT(iter != config.end(), "DagProxy: `node_name` not found in config");
  ...
  void forward(const std::vector<dict>& input_output) override {
    ...
    HAMI_ASSERT(injected_dependency_, "DagProxy: injected_dependency_ is nullptr");
