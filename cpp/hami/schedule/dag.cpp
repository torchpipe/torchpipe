...
namespace hami
{
        ...
        const dict &dict_config)
    {
        ...
        HAMI_ASSERT(iter != dict_config->end(),
                    "DagDispatcher: config not found in dict_config");
        ...
        for (const auto &item : dual_config)
        {
            ...
            HAMI_ASSERT(back, "DagDispatcher: instance(" + ("node." + item.first) +
                                  ") not found");
            base_dependencies_["node." + item.first] = back;

            auto dag_backend =
                init_backend("DagProxy", {{"DagProxy::dependency", item.first}}, {},
                             "dag." + item.first);
        ...
        for (std::size_t i = 0; i < N; ++i)
        {
            ...
            threads_.emplace_back(std::thread(&DagDispatcher::task_loop, this, i,
                                              task_queues_[i].get()));
        }
    }
                                       ...
                                       std::shared_ptr<Stack> pstack)
    {
        ...
        {
            // SPDLOG_INFO("Processed : {}. waiting {} total {}", node_name, pstack->dag.waiting_nodes.size(),
            //             pstack->dag.total);
    ...
    void DagDispatcher::execute(std::string node_name,
                                std::shared_ptr<Stack> pstack, dict curr_data)
    {
        auto curr_event = make_event();
        ThreadSafeQueue<dict> *local_queue =
            task_queues_[pstack->task_queue_index].get();
        curr_event->set_final_callback(
            [local_queue, curr_data, pstack, node_name]()
            {
                (*curr_data)[TASK_STACK_KEY] = pstack;
                (*curr_data)[TASK_NODE_NAME_KEY] = node_name;
