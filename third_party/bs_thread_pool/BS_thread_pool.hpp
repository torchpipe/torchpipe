...
#ifdef __cpp_concepts
#define BS_THREAD_POOL_IF_PAUSE_ENABLED template <bool P = pause_enabled> \
    requires(P)
    template <typename F>
...
#define BS_THREAD_POOL_PROMISE_MEMBER_ACCESS promise->
        ...
        template <typename R, typename P>
        bool wait_atmost_queued_tasks_for(const size_t max_queued, const std::chrono::duration<R, P> &duration)
        {
#ifdef __cpp_exceptions
            if constexpr (wait_deadlock_checks_enabled)
            {
            ...
            std::unique_lock tasks_lock(tasks_mutex);
            // waiting = true;
            const bool status = tasks_done_one_cv.wait_for(tasks_lock, duration,
                                                           [this, max_queued]
                                                           {
                                                               if constexpr (pause_enabled)
