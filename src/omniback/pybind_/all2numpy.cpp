...
namespace detail {
template <>
struct type_caster<omniback::Bytes> {
 public:
  PYBIND11_TYPE_CASTER(omniback::Bytes, _("Bytes"));

  // Python -> C++ 转换
  bool load(handle src, bool convert) {
    if (py::isinstance<py::bytes>(src)) {
      value = omniback::Bytes(py::cast<py::bytes>(src)).data();
      return true;
    }
    return false;
  }
