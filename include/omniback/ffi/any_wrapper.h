...
class Any {
 ...
 public:
  ...
  std::optional<T> try_cast() const {
    ...
    if constexpr (std::is_unsigned_v<TDecay> && sizeof(TDecay) == sizeof(uint64_t))
      {
    ...
    } else if constexpr (is_tvm_type_v<T> && !is_om_type_v<T>) {
      auto& value = std::get<tvm::ffi::Any>(storage_);
      return value.try_cast<TDecay>();
    } else {
      return std::get<StdAny>(storage_)->try_cast<TDecay>();
    }
  }
  ...
  T cast() const {
    ...
    if constexpr (
      ...
      return std::get<StdAny>(storage_)->cast<T>();
    }
  }
