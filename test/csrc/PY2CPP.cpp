...
class PY2CPP : public ipipe::Backend {
 public:
  void forward(const std::vector<ipipe::dict>& input_dicts) override {
    ...
    at::Tensor Tensor = any_cast<at::Tensor>(data["torch.Tensor"]);
...
class ListPY2CPP : public ipipe::Backend {
 public:
  void forward(const std::vector<ipipe::dict>& input_dicts) override {
    ...
    std::vector<at::Tensor> Tensor = any_cast<std::vector<at::Tensor>>(data["torch.Tensor"]);
...
class SetPY2CPP : public ipipe::Backend {
 public:
  void forward(const std::vector<ipipe::dict>& input_dicts) override {
    ...
    // std::unordered_set<at::Tensor> Tensor =
    //     any_cast<std::unordered_set<at::Tensor>>(data["torch.Tensor"]);
...
class StrMapPY2CPP : public ipipe::Backend {
 public:
  void forward(const std::vector<ipipe::dict>& input_dicts) override {
    ...
    StrMap<at::Tensor> Tensor = any_cast<StrMap<at::Tensor>>(data["torch.Tensor"]);
