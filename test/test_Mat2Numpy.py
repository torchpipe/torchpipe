...
class TestMat2Numpy:
    ...
    @pytest.mark.parametrize("src_array", src_arrays)
    def test_ReturnMat(self, target_h, target_w, src_array):
        ...
        pipe_tensor = torchpipe.pipe({"a": {"resize_h": target_h, "resize_w": target_w, "color": 'bgr',
                                            "save_dir": "./",
                                            'backend': "Sequential[Tensor2Mat,SyncTensor]"}})

        pipe_tensor(imnpoit)
        assert (src_array.shape == imnpoit["result"].shape)
        zzz = src_array-imnpoit["result"]  # .astype(np.float32)
        assert (src_array.dtype == imnpoit["result"].dtype)
        assert (np.max(np.fabs(zzz)) == 0)
        return
...
if __name__ == "__main__":
    ...
    if True:
        for i in range(len(a.src_arrays)):
            print(i)
            # if i==0:
            #     continue
            a.test_ReturnMat(1, 1, a.src_arrays[i])
    else:
