...
#include "omniback/csrc/all2numpy.hpp"

#include <pybind11/stl.h>

#include "omniback/core/queue.hpp"
#include "omniback/csrc/converts.hpp"
...
PyDict::PyDict(const py::dict& data) {
  ...
  for (const auto& item : data) {
    ...
    if (second == std::nullopt) {
      throw py::type_error("omniback.Any: The input type is unknown. (PyDict)");
    }
...
PyDict::PyDict(dict data) : data_(data) {
  OMNI_ASSERT(data_ != nullptr, "The input data is nullptr.");
}
...
dict PyDict::py2dict(py::dict data) {
  ...
  for (const auto& item : data) {
    ...
    if (second == std::nullopt) {
      throw py::type_error("omniback.Any: The input type is unknown.");
    }
...
void PyDict::set(const std::string& key, const py::object& value) {
  ...
  if (data == std::nullopt)
    throw py::type_error("The input type is unknown by omniback.Any.");
...
void init_dict(py::module_& m) {
  py::class_<PyDict> omniback_dict(m, "Dict");

  omniback_dict.doc() =
      "omniback.Dict provides an object wrapper for the "
      "omniback::dict class, which is essentially a wrapper around "
      "std::shared_ptr<std::unordered_map<std::string, std::any>>.";
  omniback_dict.def(py::init<>())
      .def(
  ...
  py::register_exception<omniback::queue::QueueException>(m, "QueueError");
  ...
  try {
    ...
    py::exception<omniback::queue::QueueEmptyException>(
        m, "Empty", py_empty.ptr());
    py::exception<omniback::queue::QueueFullException>(
        m, "Full", py_full.ptr());
  } catch (...) {
    ...
    SPDLOG_ERROR(
        ...
        "exceptions.");
    py::register_exception<omniback::queue::QueueEmptyException>(m, "Empty");
    py::register_exception<omniback::queue::QueueFullException>(m, "Full");
  }

  m.def("print", [](const std::string& info) {
    py::gil_scoped_release release;

    omniback::default_logger_raw()->info(info);
  });
