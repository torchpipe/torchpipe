 ...
 *
 */
class InstanceHandler : public Backend {
 public:
  ~InstanceHandler() {
  ...
  /**
   * @brief 初始化，并启动执行线程 InstanceHandler::run.
   *
   * @param backend 真正的运算后端
   * @param _independent_thread_index 实例编号。
   * @param _batched_queue ThreadSafeQueue<std::vector<dict>>*
   * 数据队列，可以从中获取数据。
   *
  ...
  bool init(const std::unordered_map<std::string, std::string>& config, dict dict_config) {
    ...
    batched_queue_ =
        any_cast<ThreadSafeQueue<std::vector<dict>>*>(dict_config->at("_batched_queue"));
    ...
    thread_ = std::thread(&InstanceHandler::run, this);
  ...
  void forward(const std::vector<dict>& input_dicts) override final {
    ...
    batched_queue_->Push(input_dicts);
