...
class SizedQueue {
 ...
 public:
  ...
  T& front() { return queue_.front().first; }

  std::size_t front_size() const { return queue_.front().second; }

  // Returns the value of the first element in the queue. This version of the function is const.
  const T& front() const { return queue_.front().first; }
};
}  // namespace ipipe