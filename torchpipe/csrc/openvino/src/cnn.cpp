...
void CnnDLSDKBase::Load() {
  ...
  ppp.input().tensor().set_layout(m_desired_layout);
  ppp.input().preprocess().convert_layout(model_layout).convert_element_type(ov::element::f32);
  ...
  m_compiled_model = m_config.m_core.compile_model(
      model, m_config.m_deviceName,
      ov::hint::performance_mode(ov::hint::PerformanceMode::THROUGHPUT),
      // ov::streams::num(instance_num), ov::inference_num_threads(instance_num),
      ov::hint::num_requests(m_config.instance_num), ov::hint::allow_auto_batching(false));

  logCompiledModelInfo(m_compiled_model, m_config.m_path_to_model, m_config.m_deviceName,
                       m_config.m_model_type);
...
void VectorCNN::Compute(const std::vector<cv::Mat>& images, std::vector<cv::Mat>* vectors,
  ...
  vectors->clear();
  auto results_fetcher = [vectors, outp_shape](const std::map<std::string, ov::Tensor>& outputs,
                                               size_t batch_size) {
    for (auto& output : outputs) {
      ov::Tensor tensor = output.second;
      ov::Shape shape = tensor.get_shape();
      if (shape.size() == 4) {
        std::vector<int> tensor_sizes(shape.size() - 1, 0);
        if (shape[0] != 1) {
          throw std::runtime_error("Invalid batch size(!=1) in output");
        }
        int w = shape[2];
        int h = shape[1];
        cv::Mat out_tensor(std::vector<int>({w, h}), CV_32FC3, tensor.data<float>());
        vectors->emplace_back(out_tensor);
      } else if (shape.size() == 2) {
        cv::Mat out_tensor(shape[1], 1, CV_32F, tensor.data<float>());
        vectors->emplace_back(out_tensor);
      } else {
      ...
      //   cv::Mat tensor_wrapper(
      //       out_tensor.size[1], 1, CV_32F,
      //       reinterpret_cast<void*>((out_tensor.ptr<float>(0) + b * out_tensor.size[1])));
      //   vectors->emplace_back();
      //   if (!isSizeEmpty(outp_shape)) tensor_wrapper = tensor_wrapper.reshape(1,
      //   outp_shape.height); tensor_wrapper.copyTo(vectors->back());
      // }
    }
  };
...
int VectorCNN::maxBatchSize() const { return m_config.m_max_batch_size; }
