...
class VectorCNN : public CnnDLSDKBase {
 public:
  ...
  void Compute(const cv::Mat& image, cv::Mat* vector, cv::Size outp_shape = cv::Size());
  void Compute(const std::vector<cv::Mat>& images, std::vector<cv::Mat>* vectors,
