...
inline slog::LogStream& operator<<(slog::LogStream& os, const ov::Version& version) {
  return os << "OpenVINO" << slog::endl
            << "\tversion: " << OPENVINO_VERSION_MAJOR << "." << OPENVINO_VERSION_MINOR << "."
...
class Color {
 ...
 public:
  ...
  inline unsigned char red() const { return _r; }

  inline unsigned char blue() const { return _b; }

  inline unsigned char green() const { return _g; }
};
...
inline void logCompiledModelInfo(const ov::CompiledModel& compiledModel,
                                 const std::string& modelName, const std::string& deviceName,
                                 const std::string& modelType = "") {
  slog::info << "The " << modelType << (modelType.empty() ? "" : " ") << "model " << modelName
    ...
    for (const auto& device : devices) {
      try {
        ...
        if (device == "CPU") {
          ...
          slog::info << "\t\tNumber of threads: "
                     << (nthreads == 0 ? "AUTO" : std::to_string(nthreads)) << slog::endl;
        }
...
inline void logBasicModelInfo(const std::shared_ptr<ov::Model>& model) {
  ...
  for (const ov::Output<ov::Node>& input : inputs) {
    ...
    slog::info << "\t\t" << name << ", " << type << ", " << shape << ", " << layout.to_string()
               << slog::endl;
  }
