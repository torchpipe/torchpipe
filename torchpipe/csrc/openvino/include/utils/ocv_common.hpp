...
static UNUSED void matToTensor(const cv::Mat& mat, const ov::Tensor& tensor, int batchIndex = 0) {
  ...
  cv::Mat resizedMat;
  if (static_cast<int>(width) != mat.size().width ||
...
static UNUSED ov::Tensor wrapMat2Tensor(const cv::Mat& mat) {
  ...
  const bool isDense = !isMatFloat ? (strideW == channels && strideH == channels * width)
                                   : (strideW == channels * sizeof(float) &&
                                      strideH == channels * width * sizeof(float));
  if (!isDense) {
...
static inline void resize2tensor(const cv::Mat& mat, const ov::Tensor& tensor) {
  ...
  cv::Size size{int(shape[ov::layout::width_idx(layout)]),
                int(shape[ov::layout::height_idx(layout)])};
  assert(tensor.get_element_type() == ov::element::f32);
  assert(shape.size() == 4);
  assert(shape[ov::layout::batch_idx(layout)] == 1);
  assert(shape[ov::layout::channels_idx(layout)] == 3);
  cv::resize(mat, cv::Mat{size, CV_32FC3, tensor.data()}, size);
}
...
static inline std::tuple<bool, ov::Layout> makeGuesLayoutFrom4DShape(
...
/**
 * @brief Puts text message on the frame, highlights the text with a white border to make it
 ...
 */
inline void putHighlightedText(const cv::Mat& frame, const std::string& message, cv::Point position,
                               int fontFace, double fontScale, cv::Scalar color, int thickness) {
  cv::putText(frame, message, position, fontFace, fontScale, cv::Scalar(255, 255, 255),
              thickness + 1);
...
static inline bool isSizeEmpty(const cv::Size& size) { return size.width <= 0 || size.height <= 0; }

// TODO: replace with Rect::empty() after OpenCV3 is dropped
static inline bool isRectEmpty(const cv::Rect& rect) { return rect.width <= 0 || rect.height <= 0; }
...
class OutputTransform {
 public:
  ...
  cv::Size computeResolution() {
    ...
    scaleFactor =
        std::min(outputResolution.height / inputHeight, outputResolution.width / inputWidth);
    newResolution = cv::Size{static_cast<int>(inputWidth * scaleFactor),
