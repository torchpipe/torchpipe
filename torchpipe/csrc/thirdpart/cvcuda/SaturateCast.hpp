 ...
 *
 * @details This function saturate casts (clamping with potential rounding) all elements to the
 * range defined by the template argument type \p T.  For instance, a float4 with any values (can be
 * below 0 and above 255) can be casted to an uchar4 rounding-then-saturating each value to be in
 * between 0 and 255 (see example below).  It is a requirement of SaturateCast that both types have
 * type traits and type \p T must be a regular C type.
 *
