...
// Leaf-node saturate cast implementation accepts all-combination types and is always used in
// host-side calls and sometimes used in device-side calls, i.e. when there is no fast
// specialization to be used instead (see below)
...
// Internal-node saturate cast implementations are specialized for each combination of target type
// and source type and uses inline PTX assembly in device-side calls and the base saturate cast
// implementation in host-side calls
...
#else
#define NVCV_CUDA_SAT_BODY(TARGET_TYPE, PTX_TYPE, PTX_ASM) \
  return BaseSaturateCastImpl<TARGET_TYPE>(u)
#endif

#define NVCV_CUDA_SAT_DEF(TARGET_TYPE, SOURCE_TYPE, PTX_TYPE, PTX_ASM)                        \
  template <>                                                                                 \
  __host__ __device__ __forceinline__ TARGET_TYPE SaturateCastImpl<TARGET_TYPE, SOURCE_TYPE>( \
      SOURCE_TYPE u) {                                                                        \
