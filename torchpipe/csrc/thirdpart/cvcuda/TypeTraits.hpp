...
/**
 * @brief Metastruct to define type traits for regular C types and CUDA built-in vector (or
 * compound) types
 *
 * @details CUDA built-in vector types are also called compound types.  The number of components in
 * the metastruct is zero for regular C types and between 1 and 4 for CUDA compound types.  On the
 * flip side, the number of elements is between 1 and 4 for regular C types and CUDA compound types,
 * i.e. the number of elements may be used regardless of the type.  The base type of a CUDA compound
 * type is the type of each of its components, for instance uchar4 has 4 elements of base type
 * unsigned char.  Type traits also provide the name of the type, each element minimum and maximum
...
// @brief Metatype to serve as a requirement for a template object to meet the given boolean
// expression.
template <bool B>
 ...
 *
 * @details When number of components is zero, it yields the identity (regular C) type, and when it
 * is between 1 and 4 it yields the CUDA compound type.
 *
 * @code
 * using RGB8Type = MakeType<unsigned char, 3>; // yields uchar3
 * @endcode
 *
 * @note Note that T=char might yield uchar1..4 types when char is equal unsigned char, i.e.
 * CHAR_MIN == 0.
 *
 ...
 *
 * @details The value may be of CUDA compound type with 1 to 4 elements, where the corresponding
 * element index is 0 to 3, and the return is a reference to the element with the base type of the
 * compound type, copying the constness (that is the return reference is constant if the input value
 * is constant).  The value may be a regular C type, in which case the element index is ignored and
 * the identity is returned.  It is a requirement of the GetElement function that the type \p T has
 * type traits.
 *
 * @code
 ...
 *
 * @details Set all elements to the value \p x passed as argument.  For instance, an int3 can have
 * all its elements set to zero by calling SetAll and passing int3 as template argument and zero as
 * argument (see example below). Another way to set all elements to a value is by using the type of
 * the argument as base type and passing the number of channels of the return type (see example
 * below).
 *
 * @code
 ...
 *
 * @details Unfortunately typeid().name() in C/C++ typeinfo yields different names depending on the
 * platform.  This function returns the name of the type resembling the CUDA compound type, that may
 * be useful for debug printing.
 *
 * @code
 ...
 *
 * @details The pixel may be a CUDA compound type with 1 to 4 components.  This operator returns the
 * output stream changed by an additional string with the name of the type followed by each
 * component value in between parentheses.
 *
 * @code
 * DataType pix = ...;
 * std::cout << pix;
