...
class PillowResize {
 protected:
   ...
   */
  [[nodiscard]] static cv::Mat _nearestResample(const cv::Mat& im_in, int x_size, int y_size,
                                                const cv::Vec4f& rect);
    ...
    for (int xx = 0; xx < im_out.size().width; ++xx) {
      ...
      for (int c = 0; c < im_in.channels(); ++c) {
        ...
        for (int x = 0; x < xmax; ++x) {
          ...
          // if (yy == 44 && xx == 39 && x >= xmax - 1) {
          //   auto zz = static_cast<intmax_t>(ss) >> precision_bits;
          //   auto a = im_in.ptr<T>(static_cast<int>(yy) + offset, static_cast<int>(x) + xmin)[c];
          //   auto b = k[x];
          //   auto cc = 1;
          // }
        }
