...
#ifndef PillowResizeV2_HPP
#define PillowResizeV2_HPP
...
// #include "PillowResizeCudaV2.hpp"
/**
 * \brief PillowResizeV2 Porting of the resize methods from Pillow library
...
class PillowResizeV2 {
 protected:
  ...
  static inline constexpr auto _clip8_lut = _lut<Length>([](size_t n) -> uchar {
   ...
   */
  static void _resampleHorizontal(cv::Mat& im_out, const cv::Mat& im_in, int offset, int ksize,
   ...
   * \param[in] prekk Interpolation filter coefficients.
   */
  static void _resampleVertical(cv::Mat& im_out, const cv::Mat& im_in, int ksize,
                                const std::vector<int>& bounds, const std::vector<double>& prekk);
  ...
  static void _resampleHorizontal(cv::Mat& im_out, const cv::Mat& im_in, int offset, int ksize,
                                  ...
                                  double init_buffer = 0., outMapFn<T> outMap = nullptr);
   ...
   */
  [[nodiscard]] static cv::Mat _resample(const cv::Mat& im_in, int x_size, int y_size,
 ...
 public:
   ...
   */
  [[nodiscard]] static cv::Mat resize(const cv::Mat& src, const cv::Size& out_size, int filter,
                                      const cv::Rect2f& box);
...
template <typename T>
void PillowResizeV2::_resampleHorizontal(cv::Mat& im_out, const cv::Mat& im_in, int offset,
                                         int ksize, const std::vector<int>& bounds,
                                         const std::vector<double>& prekk,
                                         preprocessCoefficientsFn preprocessCoefficients,
                                         double init_buffer, outMapFn<T> outMap) {
  ...
  for (int yy = 0; yy < im_out.size().height; ++yy) {
    for (int xx = 0; xx < im_out.size().width; ++xx) {
      ...
      for (int c = 0; c < im_in.channels(); ++c) {
        ...
        for (int x = 0; x < xmax; ++x) {
          // NOLINTNEXTLINE
          ss += im_in.ptr<T>(static_cast<int>(yy) + offset, static_cast<int>(x) + xmin)[c] * k[x];
        }
        // NOLINTNEXTLINE
