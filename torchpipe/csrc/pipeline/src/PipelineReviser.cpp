...
bool PipelineReviser::init(const std::unordered_map<std::string, std::string>& config,
                           dict dict_config) {
  // auto iter = config.find("PipelineReviser::backend");
  // if (iter == config.end()) {
  //   SPDLOG_ERROR("PipelineReviser::backend not exists");
  //   return false;
  // }
...
void PipelineReviser::forward(const std::vector<dict>& inputs) {
#if 0
...
IPIPE_REGISTER(Backend, PipelineReviser, "PipelineReviser");
