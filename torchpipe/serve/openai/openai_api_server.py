...
# This server is adapted from fastchat.serve.openai_api_server with a local worker function."""
import asyncio
import argparse
import json
import os
...
from local_torchpipe_worker import create_background_tasks

logger = build_logger("openai_api_server", "_openai_api_server.log")
...
import local_torchpipe_worker
async def fetch_remote(url, args={}, name=None):
    
    method_to_call = getattr(local_torchpipe_worker, url)
    
 
    result = await method_to_call(args)
     
    return result[name] if name else result

async def fetch_remote_generate(url, args={}):
    
    method_to_call = getattr(local_torchpipe_worker, url)
    
    async for data in method_to_call(args):
        yield data
...
async def check_length(request, prompt, max_tokens, worker_addr):
    ...
    context_len = await fetch_remote(
        "model_details", {"model": request.model}, "context_length"
    )
...
async def get_worker_address(model_name: str) -> str:
    ...
    worker_addr = await fetch_remote(
        "get_worker_address", {"model": model_name}, "address"
    )
...
async def get_conv(model_name: str, worker_addr: str):
    ...
    if conv_template is None:
        conv_template = await fetch_remote(
            "worker_get_conv_template", {"model": model_name}, "conv"
        )
...
async def show_available_models():
    raise NotImplementedError("This API is not implemented yet.")
    
    model_cards = [ModelCard(id="", root="", permission=[ModelPermission()])]
    return ModelList(data=model_cards)

    controller_address = app_settings.controller_address
    ret = await fetch_remote("refresh_all_workers")
    models = await fetch_remote("list_models", None, "models")
...
async def chat_completion_stream_generator(
    ...
    for i in range(n):
        ...
        previous_text = ""
        gen_params['request_id'] = id
        async for content in generate_completion_stream(gen_params, worker_addr):
            print(content["error_code"], 'error_code')
            if content["error_code"] != 0:
...
async def create_completion(request: CompletionRequest):
    ...
    if request.stream:
        generator = generate_completion_stream_generator(
            request, request.n, worker_addr, id
        )
        
        return StreamingResponse(generator, media_type="text/event-stream", background=create_background_tasks(id))
    else:
        text_completions = []
        for text in request.prompt:
            gen_params = await get_gen_params(
...
async def generate_completion_stream_generator(
    request: CompletionRequest, n: int, worker_addr: str, id: str
):
    ...
    for text in request.prompt:
        for i in range(n):
            ...
            async for content in generate_completion_stream(gen_params, worker_addr):
                ...
                print(content["error_code"], delta_text)
...
async def generate_completion_stream(payload: Dict[str, Any], worker_addr: str):
    controller_address = app_settings.controller_address
    async for content in fetch_remote_generate("worker_generate_stream", payload):
        yield content
...
async def generate_completion(payload: Dict[str, Any], worker_addr: str):
    return await fetch_remote("worker_generate", payload, None)
...
def create_openai_api_server():
    ...
    parser.add_argument("--port", type=int, default=11112, help="port number")
    parser.add_argument(
