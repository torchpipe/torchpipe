...
def save_embed_tokens(model, save_dir = 'onnx/'):
    with torch.no_grad():
...
def save_batchful(model, num_layers = None, save_path = 'onnx/', return_last_seq_len = False):
...
def save_prefill_batchless(model, save_path = 'onnx/'):
    llama_model = model.model
...
if __name__ == '__main__':
    ...
    MODEL_EXPORT = os.getenv('MODEL_EXPORT', '1')
    print(MODEL_EXPORT)
    MODEL_EXPORT = MODEL_EXPORT == '1'
    
    # 加载模型和分词器
    tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-chat-hf")
    model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b-chat-hf",attn_implementation='eager', torch_dtype = "float16")
    model.eval()
    ...
    # save_embed_tokens(model)
    return_last_seq_len = True
    if MODEL_EXPORT:
        save_batchful(model, 2, 'onnx/', return_last_seq_len=True)
    
    if MODEL_EXPORT:
        save_prefill_batchless(model)
        exit(0)
    ## model.to('cuda:0')
    if not MODEL_EXPORT:
        text = "San Francisco is a"
        device = model.model.device # torch.device("cuda" if torch.cuda.is_available() else "cpu")
