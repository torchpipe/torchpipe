...
def window_partition(x, window_size):
    B, C, H, W = x.shape
    x = x.view(-1, int(C), int(H // window_size), window_size, int(W // window_size), window_size)
    
    scale = int(C* H* W)/(window_size*window_size* int(C))
    print(x.shape, "window_partition: x")
    cc = int(x.numel()/(window_size*window_size*C*B))
    windows = x.permute(0, 2, 4, 3, 5, 1).reshape(-1, window_size*window_size, int(C))
    # todo  batch dim keeps 0
    print(windows.shape, "window_partition: view")
...
def window_reverse(windows, window_size, H, W):
    ...
    sym_B=windows.shape[0] / (H * W / window_size / window_size)
    print(windows.shape, B, "BBBBB")
    # x = windows.reshape(B, H // window_size, W // window_size, window_size, window_size, -1)
    x = windows.reshape(-1, int(H // window_size), int(W // window_size), window_size, window_size, int(windows.shape[-1]))
    assert(int(windows.shape[0] / (H * W / window_size / window_size)) == x.shape[0])
    # print(x.shape, "reshape xxx")
    x = x.permute(0, 5, 1, 3, 2, 4).reshape(-1,int(windows.shape[2]), H, W)
    assert(x.shape[0]==B)
    return x
...
def ct_dewindow(ct, W, H, window_size):
    ...
    # ct2 = ct2.reshape(-1, N, W, H).flatten(2).transpose(1, 2)
    ct2 = ct2.reshape(-1, N, W*H).transpose(1, 2)
    return ct2
    ...
    def forward(self, input_tensor):
        ...
        if not self.grid_exists:
            ...
            if self.rank == 1:
                ...
                relative_coords_table /= (seq_length // 2)
                if not self.conv:
                    # print("PosEmbMLPSwinv1D", self.rank,self.grid_exists)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        # self.re2 = nn.Sigmoid()
    def forward(self, x):
        # print(x.shape, "WindowAttention")
        B, N, C = x.shape
        C=int(C)
        
        x=self.qkv(x)
        # print(x.shape, "WindowAttention")
        target = int(x.shape[0]*x.shape[1]*x.shape[2]/(B*3*C))
        qkv = x.reshape(-1, target, 3, self.num_heads, C // self.num_heads).permute(0, 2, 3, 1, 4)
        assert(qkv.shape[1] == 3)
        
        q, k, v = qkv[:,0,...], qkv[:,1,...], qkv[:,2,...]
        # print(q.shape,qkv.shape, " qkv WindowAttention")
...
class HAT(nn.Module):
    ...
    def forward(self, x, carrier_tokens):
        ...
        N=int(N)
        ct = carrier_tokens
        print(x.shape, "self.pos_embed.in")
        x = self.pos_embed(x)
        ...
        if self.sr_ratio > 1:
            ...
            IN = (ct.shape[0]*ct.shape[1]*ct.shape[2])/(x.shape[0]*N)
            ct = ct.reshape(-1, int(IN), N)
            assert(ct.shape[0]==x.shape[0])
            # print("xxxx--ct ", ct.shape)
        ...
        if self.sr_ratio > 1:
            ...
            ct = ctr.reshape(-1, Ng, Hg) # reshape carrier tokens.
            if self.last and self.do_propagation:
                # propagate carrier token information into the image
                ctr_image_space = ctr.transpose(1, 2).reshape(-1, N, self.cr_window, self.cr_window)
...
class TokenInitializer(nn.Module):
    ...
    def __init__(self,
        ...
        # print("TokenInitializer ", output_size, stride_size, kernel_size)

    def forward(self, x):
        x = self.to_global_feature(x)
        B, C, H, W = x.shape
        
        ct = x.view(-1, C, int(H // self.window_size), int(self.window_size), int(W // self.window_size), int(self.window_size))
        # x=self.re (x)
        ct = ct.permute(0, 2, 4, 3, 5, 1)
        ct=ct.reshape(-1, int(H*W), int(C))
        print("TokenInitializer ",ct.shape, self.window_size)
        return ct
...
class FasterViTLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if conv:
            ...
            print("FasterViTLayer: ", only_local,input_resolution, ct_size)
            self.blocks = nn.ModuleList([
                HAT(dim=dim,
                    num_heads=num_heads,
                    mlp_ratio=mlp_ratio,
    ...
    def forward(self, x):
        ...
        true_batch = int(B)
        # print(x.shape, "before xxx")
        if self.transformer_block:
            # print(x.shape, "window_partition xxx", 1)
            x = window_partition(x, self.window_size)
            
            
            # print(x.shape, "after window_partition")
            
        for bn, blk in enumerate(self.blocks):
            x, ct = blk(x, ct)
            print(x.shape, "blk xxx", bn)
        print("-------")
        if self.transformer_block:
            x = window_reverse(x, self.window_size, H, W)
            print(x.shape, "after window_reverse")
