...
# update 0.2.1b3 2022-09-08  增加callback， 用于接收和处理结果；此时推理函数返回类型需要是list类型；
...
def pre_resize(im, max_size=640.):
    h, w, _ = im.shape
    if max(h, w) <= max_size:
        return im
...
class InferThread(threading.Thread):
    def __init__(self, index, params, infer, tran_list, test_params:TestParams, error_time_out=5000) -> None:
    ...
    def forward(self, input, out_print=True):
        ...
        if (out_print and random.random() < self.print_prob and result is not None) or result_time > self.error_time_out:
            with self.params[1]:
        ...
        if result_time > self.error_time_out:
            print("response too slow: ", result_time,
                  f"data left: {self.params[0]}")
        return result_time
...
def test_from_raw_data(run, img_dir, request_client=10, request_batch=1, total_number=1*1*10000, pid=0, extra_post=[]):
    test_from_raw_jpg(run, img_dir, request_client, request_batch, total_number, pid,   [".png", ".bmp"])
# todo request_batch=(0,1,2,3) or [0,1,2,3]
def test_from_raw_jpg(run, img_dir, request_client=10, request_batch=1, total_number=1*1*10000, pid=0,extra_post=[], call_back=[]):
    tran_list = []
    img_root = img_dir
    if not os.path.exists(img_root):
        print(img_dir, " not exists")
    # get images list
    print(f"\nstart testing, request_client={request_client}, request_batch={request_batch}, total_number={total_number}", flush=True)
    local_max_pic_loaded = -1   
    if total_number > 0:  
        local_max_pic_loaded = min(max_pic_loaded, total_number)
        print(
            f'preloading first {local_max_pic_loaded} images from {img_root} and its subdirectories', flush=True)
    ...
    params = [total_number, threading.Lock(), list_latency, request_batch,
              print_prob, shared_print_prob]

    if isinstance(run, list):
        instance_threads = [InferThread(
            i, params, run[i], tran_list, test_params) for i in range(request_client)]
    else:
        instance_threads = [InferThread(
            i, params, run, tran_list, test_params) for i in range(request_client)]
    ...
    resource_thread = ResourceThread(
        pid, resource_result, resource_event)
    ...
    print(f"throughout::     qps:  {qps},   [qps:=total_number/total_time]")
    print(
...
class ThriftInfer():
    ...
    def infer(self, data):
        return self.client.infer_batch(self.InferenceParams(data[0][0],data[0][1]))

    def __del__(self):
        self.transport.close()


def test_thrift_from_raw_jpg(img_dir, host="localhost", port=8095, request_client=10, request_batch=1, total_number=1*1*10000):

    instances_ = [ThriftInfer(host, port, request_batch)
                  for i in range(request_client)]

    test_from_raw_jpg([x.infer for x in instances_], img_dir,
                      request_client, request_batch, total_number)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--host', type=str,
                        help='Host to run service', default='localhost')
    parser.add_argument('--img_dir', type=str,
                        help=f'img path. 预读取该目录以及子目录下至多{max_pic_loaded}张图片', default='./img/')
    parser.add_argument('--request_batch', type=int,
                        help='单次请求的数据量', default=1)
    parser.add_argument('--request_client', type=int, help='并发请求数', default=20)
...
if __name__ == "__main__":
    ...
    test_thrift_from_raw_jpg(args.img_dir, host=args.host, port=args.port,
                             request_client=args.request_client, request_batch=args.request_batch, total_number=total_number)
