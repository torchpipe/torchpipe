...
constexpr int BBOX_CONF_THRESH = 25;
...
class PostProcYoloV8 : public Backend {
 ...
 protected:
  std::vector<std::vector<Object>> forward_impl(std::vector<at::Tensor> net_outputs,
    ...
    nms_sorted_bboxes_class_specific(bboxes, picked, NMS_THRESH / 100., num_classes);
#else
    nms_sorted_bboxes(bboxes, picked, NMS_THRESH / 100.);
#endif
