...
cv::RotatedRect PostProcessor::UnClip(
...
float PostProcessor::PolygonScoreAcc(
  ...
  int xmin = clamp(
      int(std::floor(*(std::min_element(box_x.begin(), box_x.end())))),
      0,
      width - 1);
  int xmax = clamp(
      int(std::ceil(*(std::max_element(box_x.begin(), box_x.end())))),
      0,
      width - 1);
  int ymin = clamp(
      int(std::floor(*(std::min_element(box_y.begin(), box_y.end())))),
      0,
      height - 1);
  int ymax = clamp(
      int(std::ceil(*(std::max_element(box_y.begin(), box_y.end())))),
      0,
      height - 1);
...
std::vector<std::vector<std::vector<int>>> PostProcessor::FilterTagDetRes(
  ...
  for (int n = 0; n < boxes.size(); n++) {
    ...
    if (rect_width <= 8 || rect_height <= 8)
      continue;
    auto fac = fabs(1.0 * rect_width - rect_height) /
        std::min(rect_height, rect_width);
    if (fac < 0.3 && rect_height * rect_width < 32 * 32) {
      int x_collect[4] = {
          boxes[n][0][0], boxes[n][1][0], boxes[n][2][0], boxes[n][3][0]};
      int y_collect[4] = {
          boxes[n][0][1], boxes[n][1][1], boxes[n][2][1], boxes[n][3][1]};
      int left = int(*std::min_element(x_collect, x_collect + 4));
      int right = int(*std::max_element(x_collect, x_collect + 4));
      int top = int(*std::min_element(y_collect, y_collect + 4));
      int bottom = int(*std::max_element(y_collect, y_collect + 4));

      if ((right - left) * (bottom - top) * 1.0 / (rect_width * rect_height) <
          1.4) {
        if (rect_height <= 16) {
          top -= 2;
          bottom += 2;
        ...
        left = std::max(left, 0);
        right = std::min(right, oriimg_w - 1);
        top = std::max(top, 0);
        bottom = std::min(bottom, oriimg_h - 1);
        boxes[n][0][0] = left;
        boxes[n][0][1] = top;
