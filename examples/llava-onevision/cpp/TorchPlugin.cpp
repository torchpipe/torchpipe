...
int32_t TorchPlugin::enqueue(PluginTensorDesc const* inputDesc, PluginTensorDesc const* outputDesc,
  ...
  }
  {
    ...
    if (interpreter_ == nullptr) {
      config_param_ = ipipe::ThreadCacher::query_config();
      dict_config_ = ipipe::ThreadCacher::query_dict_config();
  ...
  //              (long)c10::cuda::getCurrentCUDAStream().stream());
  {
    ...
    const std::vector<ipipe::dict>& input_dicts = ipipe::ThreadCacher::query_input();
