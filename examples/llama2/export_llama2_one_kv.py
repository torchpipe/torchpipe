...
def save_decode_batchless(model, save_path = 'model_files/'):
    ...
    def batchless_forward(self, query_states, key_states, value_states, cos, sin, attention_mask, past_key , past_value):
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        past_value_out = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim)
        value_states = past_value_out.transpose(1, 2)
        
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
        assert past_key is not None
        past_key_out = key_states.transpose(1, 2)
        key_states = torch.cat([past_key.transpose(1,2), key_states], dim=2) 
        value_states = torch.cat([past_value.transpose(1,2), value_states], dim=2) 
        ...
        return attn_output, past_key_out, past_value_out
    ...
    key_states = torch.zeros((bsz, q_len, hidden_size), dtype=torch.float16, device=device)
    value_states = torch.zeros((bsz, q_len, hidden_size), dtype=torch.float16, device=device)
    past_key = torch.zeros((1,past,32 , hidden_size//32), dtype=torch.float16, device=device)
    past_value = torch.zeros((1,past,32,  hidden_size//32), dtype=torch.float16, device=device)
    cos = torch.zeros((bsz, q_len, hidden_size//32), dtype=torch.float16, device=device)
    sin = torch.zeros((bsz, q_len, hidden_size//32), dtype=torch.float16, device=device)
    attention_mask = torch.zeros((1,1, q_len, q_len_with_past), dtype=torch.float16, device=device)

    dynamic_axes={'past_key': {1: 'past'}, 'past_value': {1: 'past'},
                   'attention_mask': {3: 'q_len_with_past'}}
    # dynamic_axes.update({'past_key_output':{1: 'q_len_with_past'}, 'past_value_output':{1: 'q_len_with_past'}})
