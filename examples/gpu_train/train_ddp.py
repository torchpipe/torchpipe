...
# 目的: 将torchpipe加入到pytorch训练中，进而达到对齐gpu解码augment操作，进而可以在infer中使用gpu解码部署。
# 本代码作为 ddp 训练 的demo
...
def main_worker(args, local_rank):
    global best_acc1
    ...
    train_transform_gpu = transforms.Compose([
        ...
        # 4个参数这里设置3个，是因为最后的hue参数在1080Ti计算会导致速度变慢，其他显卡不会有问题
    ...
    val_dataset = datasets.ImageFolder(valdir, loader=cv2_loader)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(wrap_train_loader, model, criterion,
              optimizer, epoch, args, local_rank)

        # evaluate on validation set
        acc1 = validate(wrap_val_loader, model, criterion, args, local_rank)
...
def validate(val_loader, model, criterion, args, local_rank):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
        ...
        for index, (input, target) in enumerate(val_loader):
            ...
            if index % args.print_freq == 0:
                print('Test: [{0}/{1}]\t'
                      ...
                      'Acc@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                      'Acc@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                          index, len(val_loader), batch_time=batch_time, loss=losses, top1=top1, top5=top5))
