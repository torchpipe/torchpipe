    ...
    for (const auto& item : data) {
      ...
      if (py::len(item_dict) == 0) {
        ...
        for (const auto& inner_item : item_dict) {
          const std::string inner_key = inner_item.first.cast<std::string>();
          std::string inner_value = py::cast<std::string>(inner_item.second);
...
std::optional<any> object2any(const py::handle& obj) {
  ...
  if (py::isinstance<py::dict>(obj)) {
    ...
    throw py::type_error("The input data(tuple) is not supported by hami.any. Use List instead.");
  } else {
...
py::object any2object(const any& input) {
  return hami::local::any2object_from_hash_register(input);
}
...
//   void add_converter(std::function<std::optional<any>(const pybind11::handle&)> converter) {
...
//   std::vector<std::function<std::optional<any>(const pybind11::handle&)>> converters_;
// };

// void register_py2cpp(std::function<std::optional<any>(const pybind11::handle&)> converter) {
//   PyConverterRegistry::instance().add_converter(std::move(converter));
// }
// void register_cpp2py(std::function<std::optional<pybind11::object>(const hami::any&)> converter)
