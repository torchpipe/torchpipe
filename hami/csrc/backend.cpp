  ...
  void init(const std::unordered_map<string, string>& config,
    ...
    if (py::hasattr(*obj_, "init")) {
      ...
      throw std::invalid_argument(
          "No `init` method found in the python backend. You may need to use Forward[yourpython].");
    }
    if (!dict_config) {
      ...
      throw std::invalid_argument("init must have 1 or 2 arguments except self.");
    } else {
      if (init_num_params_ != 3) throw std::invalid_argument("init must have 2 arguments");
...
namespace {
static std::shared_ptr<Backend> create_backend_from_py(const std::string& class_name,
...
using ConfigVariant =
    std::variant<std::unordered_map<std::string, std::string>, std::unordered_map<std::string, int>,
                 std::unordered_map<std::string, double>>;
std::unordered_map<std::string, std::string> convert_config(const ConfigVariant& config) {
  return std::visit(
      [](auto&& arg) {
...
static std::shared_ptr<Backend> init_backend_from_py(const std::string& backend_config,
                                                     ConfigVariant variant_config,
                                                     std::optional<PyDict> dict_config_op,
                                                     py::object aspect_name = py::none()) {
  auto config = convert_config(variant_config);
  const std::string aspect_name_str =
      aspect_name.is_none() ? "" : py::cast<std::string>(aspect_name);
  dict cpp_dict_config = dict_config_op ? (*dict_config_op).to_dict() : nullptr;
  py::gil_scoped_release guard;
  auto backend =
      std::shared_ptr<Backend>(init_backend(backend_config, config, cpp_dict_config).release());
  if (!aspect_name_str.empty()) {
    // HAMI_INSTANCE_REGISTER(Backend, aspect_name_str, backend);
    register_backend(aspect_name_str, backend);
  }
...
static void register_backend(const std::string& aspect_name, py::object obj) {
  if (!py::hasattr(obj, "forward")) {
    if (py::isinstance<Backend>(obj)) {
      std::shared_ptr<Backend> backend = py::cast<std::shared_ptr<Backend>>(obj);
      // HAMI_INSTANCE_REGISTER(Backend, aspect_name, backend);
      register_backend(aspect_name, backend);
      return;
    } else {
      throw std::invalid_argument("The backend must have a forward method, or is a hami.Backend.");
    }
  }
  ...
  do {
    if (py::isinstance<py::type>(obj)) {
      ...
      throw std::invalid_argument(
          "You must provide an instance, or a type name that can be default "
          "constructed (__init__(self)).");
    }
  } while (false);

  int num_params = get_num_params(obj, "forward");
  HAMI_ASSERT(num_params == 2, "The forward function must have exactly one argument except self.");
...
void py_init_backend(py::module_& m) {
  m.def("create", &create_backend_from_py, py::arg("name"), py::arg("register_name") = py::none(),
        R"pbdoc(
      ...
      .def("init", &init_backend_from_py, py::arg("backend"), py::arg("config"),
           py::arg("dict_config") = py::none(), py::arg("register_name") = py::none(),
           R"pbdoc(
  ...
  m.def(
      "register",
      [](const std::string& name, const py::object& backend) { register_backend(name, backend); },
  ...
  // m.add_object("_cleanup", py::capsule([]() { HAMI_INSTANCE_CLEANUP(Backend); }));
  ...
  hami_backend.def(
      ...
      [](const std::shared_ptr<Backend>& self, str::str_map config,
        ...
        if (!dict_config.is_none()) {
          dict_config_dict = py::cast<const PyDict&>(dict_config).to_dict();
        }
            ...
            if (py::isinstance<PyDict>(input_output)) {
              ...
              for (const auto& item : input_output) {
                HAMI_ASSERT(py::isinstance<PyDict>(item),
                            ...
                            "the following: dict, hami.dict, or List[hami.dict].");
              ...
              if (input_output.contains(TASK_EVENT_KEY)) {
                throw py::key_error(
                    ...
                    "`]. This indicates an asynchronous call. In this case, writing "
                    "back values to the Python dictionary is not supported yet. "
              ...
              throw std::invalid_argument(
                  "unsupported input type. Try Union[dict, hami.dict, List[hami.dict]");
            }
