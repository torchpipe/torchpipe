...
#define REGISTER_CONVERTER(type) \
  g_type_names[typeid(type)] =   \
      any2obj_func([](const any& self) { return py::cast(any_cast<type>(self)); });

#define REGISTER_NO_VECTOR_CONVERTER(type)                                                         \
  g_type_names[typeid(type)] =                                                                     \
      any2obj_func([](const any& self) { return py::cast(any_cast<type>(self)); });                \
  g_type_names[typeid(std::unordered_set<type>)] = any2obj_func(                                   \
      [](const any& self) { return py::cast(any_cast<std::unordered_set<type>>(self)); });         \
  g_type_names[typeid(std::unordered_map<std::string, type>)] = any2obj_func([](const any& self) { \
    return py::cast(any_cast<std::unordered_map<std::string, type>>(self));                        \
  });

#define REGISTER_VECTOR_CONVERTER(type)                 \
  SPDLOG_DEBUG("REGISTER_VECTOR_CONVERTER {} ", #type); \
  g_type_names[typeid(std::vector<type>)] =             \
      any2obj_func([](const any& self) { return py::cast(any_cast<std::vector<type>>(self)); });
...
#define REGISTER_BYTES_CONVERTER(type)                                         \
  g_type_names[typeid(std::vector<type>)] = any2obj_func([](const any& self) { \
...
py::object any2object_from_hash_register(const any& input) {
  static auto& g_type_names = get_type_map();
  auto iter = g_type_names.find(input.type().type());
  if (iter != g_type_names.end()) {
    return iter->second(input);
  } else {
  }
  throw py::type_error("cpp2py: unregistered type: " + std::string(input.type().name()));
  return py::object();
