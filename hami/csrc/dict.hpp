...
class PyDict {
 public:
  ...
  PyDict(dict data) : data_(data) { HAMI_ASSERT(data_ != nullptr, "The input data is nullptr."); }
  ...
  py::object pop(const std::string& key, std::optional<std::string> default_value = std::nullopt);

  const std::unordered_map<std::string, hami::any>& data() const { return *data_; }
