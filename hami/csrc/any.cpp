...
#include "hami/csrc/any.hpp"
#include "hami/csrc/converts.hpp"

namespace hami {

constexpr auto set_double_error_message() {
  return "The input data is a floating-point value, but it exceeds the representable range of a "
         "C++ float. We currently do not allow automatic conversion to double. If it is "
         "necessary, please construct a default any and then call its set_double function.";
}
constexpr auto set_size_t_error_message() {
  return "The input data is a int value, but it exceeds the representable range of a "
         "C++ int. We currently do not allow automatic conversion to size_t. If it is "
         "necessary, please construct a default any and then call its set_size_t function.";
}
...
#define DEFINE_CONVERSION_FUNCTIONS(type, name)                                                    \
  def(py::init<type>())                                                                            \
      ...
      .def("as_dict_of_" #name,                                                                    \
           [](const any& self) { return any_cast<std::unordered_map<std::string, type>>(self); })
...
void init_any(py::module_& m) {
  ...
  hami_any
      .def(py::init([](py::object& obj) {
        ...
        if (re == std::nullopt) {
          throw py::type_error("The input data is not supported by hami.any.");
        }
        return *re;
      }))
      .def_property_readonly("value", [](const any& self) { return any2object(self); })
      ...
      .def("as_bytes",
           [](const any& self) {
             if (self.type() == typeid(std::vector<unsigned char>)) {
               ...
               const char* data_ptr = reinterpret_cast<const char*>(data.data());
               return py::bytes(data_ptr, data.size());
             } else if (self.type() == typeid(string)) {
               const auto& data = any_cast<string>(self);
               const char* data_ptr = reinterpret_cast<const char*>(data.data());
               return py::bytes(data_ptr, data.size());
             }
             throw py::type_error(
                 "The input data cannot be interpreted as bytes. Supported types: vector<byte>, "
