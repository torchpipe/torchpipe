...
class TritonInfer:
    def __init__(self,model_name, port=8001):
        ...
        self.url = os.environ.get("TRITON_SERVER_URL", f"localhost:{port}")
...
def get_clients(model_name, num_clients, port):
    return [TritonInfer(model_name, port) for x in range(num_clients)]


def get_clients_with_preprocess(model_name, num_clients, use_process, port):
    if not use_process:
        return [TritonWithPreprocess(model_name, port) for x in range(num_clients)]
    else:
        return [
            ProcessAdaptor(TritonWithPreprocess, (model_name, port))
