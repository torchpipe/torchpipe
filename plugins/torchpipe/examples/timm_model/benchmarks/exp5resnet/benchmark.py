...
def export_onnx(onnx_save_path, model_name):
    ...
    tp.utils.model_helper.export_n3hw(model, onnx_save_path, 448, 448)
...
if __name__ == "__main__":
    ...
    if "ensemble" in args.model :
        ...
        clients = triton_utils.get_clients(
            'ensemble_dali_resnet', args.client, args.triton_http_port+1)
        run = [x.forward for x in clients]
    elif args.model == "triton_resnet_thread":
        import triton_utils

        clients = triton_utils.get_clients_with_preprocess(
            "resnet_trt", args.client, False, args.triton_http_port+1)
        run = [x.forward for x in clients]
    elif args.model == "triton_resnet_process":
        import triton_utils

        clients = triton_utils.get_clients_with_preprocess(
            "resnet_trt", args.client, True, args.triton_http_port+1)
        run = [x.forward for x in clients]
    else:
