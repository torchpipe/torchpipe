...
def start_triton_server(model_repo_cmd, gpu_id):
    ...
    port = args.triton_http_port
    model_repo_cmd += f" --grpc-port={port+1} --http-port={port} --metrics-port={port+2}"
...
def run_cmd(cmd, gpu_id, pbar=None):
    def func():
        ...
        for i in num_clients:
            total_number = 1000 if i == 1 else 2000
            cmd_new = base_cmd + [
        ...
        for key, back_cmd in TEST.items():
            ...
            try:
                ...
                result = run_cmd(['python3', './benchmark.py',
                                  '--model', key, '--triton-http-port', f'{args.triton_http_port}'] + DEFAULT_PARAMS, gpu_id, pbar)()
