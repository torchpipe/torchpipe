    ...
    {                                                                       \
        ...
        {                                                                   \
            throw std::runtime_error(                                       \
                str::format("nvjpeg error: nvjpegStatus_t = {}, EVAL = {}", \
                            int(_e), #EVAL));                               \
        }                                                                   \
    }

    bool decode(const std::string &data, nvjpegHandle_t handle,
                nvjpegJpegState_t state, torch::Tensor &image_tensor,
                const std::string &color, const std::string &data_format)
    {
        ...
        auto re = nvjpegGetImageInfo(handle, blob, data.length(), &nComponents,
        ...
        if (data_format == "nchw")
        {
            image_tensor = torch::empty({1, nComponents, h, w}, options,
                                        torch::MemoryFormat::Contiguous);
        }
        else
        {
            image_tensor = torch::empty({h, w, nComponents}, options,
                                        torch::MemoryFormat::Contiguous);
        }
        ...
        if (NVJPEG_STATUS_SUCCESS !=
            nvjpegDecode(handle, state, blob, data.length(), target_color,
...
namespace torchpipe
{
        ...
        const hami::dict &kwargs)
    {
        ...
        CHECK_NVJPEG_RESULT(nvjpegCreateExV2(backend, &dev_allocator_async,
                                             &pinned_allocator, 0, &handle_));
