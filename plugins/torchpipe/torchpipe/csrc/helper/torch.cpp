...
void copy2ptr(torch::Tensor input, char* ptr) {
    IPIPE_ASSERT(input.is_contiguous(),
                 "copy2ptr: input tensor must be contiguous");
    cudaStream_t stream = c10::cuda::getCurrentCUDAStream();
    size_t size = input.numel() * input.element_size();
    IPIPE_ASSERT(
        cudaMemcpyAsync(ptr, input.data_ptr(), size, cudaMemcpyDeviceToDevice,
                        stream) == cudaSuccess,
...
bool torch_not_use_default_stream(bool high_prio) {
    if (c10::cuda::getCurrentCUDAStream() ==
...
torch::Tensor to_current_device(torch::Tensor input) {
    ...
    // input.is_pinned()
    return input.to(
        torch::TensorOptions().device(torch::kCUDA, -1), false, false,
...
torch::Tensor async2cpu(torch::Tensor input) {
    ...
    return input.to(options, true, false,
...
torch::Tensor tensor_permute(torch::Tensor input,
    ...
    } else if (input.sizes().size() == max_shape.size() + 1 &&
               input.size(0) == 1) {
    ...
    for (std::size_t i = 1; i < max_shape.size(); ++i) {
        if (max_shape[i] < input.sizes()[i] ||
            min_shape[i] > input.sizes()[i] || need_permute) {
            need_permute = true;
            for (std::size_t j = 1; j < max_shape.size(); ++j) {
                if (max_shape[i] >= input.sizes()[j] &&
                    min_shape[i] <= input.sizes()[j] &&
                    std::find(permute_vector.begin(), permute_vector.end(),
                              j) == permute_vector.end()) {
...
torch::Tensor try_quick_cat(std::vector<torch::Tensor> resized_inputs) {
    ...
    if (share_same_storage && is_continuous) {
        ...
        true_input = torch::empty({0}, resized_inputs[0].options())
                         .set_(resized_inputs[0].storage(),
                               resized_inputs[0].storage_offset(), sizes,
...
void check_input_tensor(const torch::Tensor& tensor, const NetIOInfo& infos) {
    ...
    for (size_t i = 0; i < infos.min.nbDims; ++i) {
        ...
        HAMI_ASSERT(tensor.is_cuda() & (infos.device == NetIOInfo::Device::GPU),
                    std::to_string(tensor.is_cuda()) + " vs " +
                        std::to_string(infos.device ==
                                       NetIOInfo::Device::GPU));  // todo
