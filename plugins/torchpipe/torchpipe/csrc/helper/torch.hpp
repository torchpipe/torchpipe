...
std::vector<torch::Tensor> get_tensors(hami::dict input_dict, const std::string& key);
...
static inline int torch_malloc_async(void* ctx, void** p, size_t size, cudaStream_t stream) {
...
static inline int torch_free_async(void* ctx, void* p, size_t size, cudaStream_t stream) {
  (void)ctx;  // Ignore the context pointer if not used
  if (p == nullptr) {
...
int static inline torch_pinned_malloc_async(void* ctx, void** p, size_t size, cudaStream_t stream) {
...
int static inline torch_pinned_free_async(void* ctx, void* p, size_t size, cudaStream_t stream) {
