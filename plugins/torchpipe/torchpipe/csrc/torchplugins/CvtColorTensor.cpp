...
void CvtColorTensor::init(const std::unordered_map<std::string, std::string>& config_param,
                          const dict& dict_config) {
  auto iter = config_param.find(TASK_COLOR_KEY);
  HAMI_ASSERT(iter != config_param.end(), "CvtColorTensor: color is not set");

  color_ = iter->second;
  HAMI_ASSERT(VALID_COLOR_SPACE.count(color_) != 0, "color must be rgb or bgr: " + color_);
}

void CvtColorTensor::forward(const dict& input_dict) {
  auto src_color = input_dict->find(TASK_COLOR_KEY);
  HAMI_ASSERT(src_color != input_dict->end(), "input dict must contain color");
  ...
  HAMI_ASSERT(VALID_COLOR_SPACE.count(input_color) != 0, input_color + " is not supported yet");

#if 1
  auto input_tensor = hami::dict_get<torch::Tensor>(input_dict, TASK_DATA_KEY);
...
HAMI_REGISTER(Backend, CvtColorTensor, "cvtColorTensor,CvtColorTensor");
