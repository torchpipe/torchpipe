...
namespace torchpipe {
class SyncTensor : public hami::Dependency {
   public:
    /**
     * @brief
     ...
     * execution to.
     * @note Usage: SyncTensor[A], Sequential[A,B,C,SyncTensor] or
     * Sequential[A,B,SyncTensor[C]]. For serial units, such as
     * Sequential[SyncTensor[A],SyncTensor[B]], it will initialize in reverse
     ...
     *
     *  @ref Sequential and other containers can ensure that the initialization
     * order of their child backends is opposite to the forward order,
     * therefore, even in complex situations, the correct stream synchronization
     * timing can be obtained.
     */
    virtual void pre_init(const std::unordered_map<std::string, std::string> &,
                          const dict &) override;
     ...
     *
     */
    virtual void forward_impl(const std::vector<dict> &, Backend *) override;
