...
void LoadTensorrtEngine::impl_init(
    ...
    if (kwargs->find(TASK_ENGINE_KEY) != kwargs->end()) {
        HAMI_ASSERT(kwargs->at(TASK_ENGINE_KEY).type() ==
                    typeid(nvinfer1::ICudaEngine *));

        nvinfer1::ICudaEngine *engine = hami::any_cast<nvinfer1::ICudaEngine *>(
            kwargs->at(TASK_ENGINE_KEY));
...
void Onnx2Tensorrt::impl_init(
    ...
    if (!model_cache_exist) {
        ...
        HAMI_ASSERT(engine_ && engine_->getNbOptimizationProfiles() ==
                                   params.instance_num,
                    std::to_string(engine_->getNbOptimizationProfiles()) +
                        " vs " + std::to_string(params.instance_num));
    } else {
        SPDLOG_INFO(
        ...
        HAMI_ASSERT(engine_ && engine_->getNbOptimizationProfiles() ==
                    ...
                    std::to_string(engine_->getNbOptimizationProfiles()) +
                        " vs " + std::to_string(params.instance_num) +
                        ". You may need to delete the cached model " +
                        params.model_cache + ". Or move it to `model`.");
    }
