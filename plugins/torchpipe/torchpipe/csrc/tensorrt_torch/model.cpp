...
namespace torchpipe
{
        ...
        const hami::dict &dict_config)
    {
        ...
        if (dict_config->find(TASK_ENGINE_KEY) != dict_config->end())
        {
            ...
            nvinfer1::ICudaEngine *engine = hami::any_cast<nvinfer1::ICudaEngine *>(dict_config->at(TASK_ENGINE_KEY));
        ...
        const hami::dict &dict_config)
    {
        ...
        if (!model_cache_exist)
        {
            ...
            HAMI_ASSERT(engine_ && engine_->getNbOptimizationProfiles() ==
                                       params.instance_num,
                        std::to_string(engine_->getNbOptimizationProfiles()) + " vs " + std::to_string(params.instance_num));
        }
        else
        {
            SPDLOG_INFO(
            ...
            HAMI_ASSERT(engine_ && engine_->getNbOptimizationProfiles() ==
                                       params.instance_num,
                        std::to_string(engine_->getNbOptimizationProfiles()) + " vs " + std::to_string(params.instance_num) + ". You may need to delete the cached model " + params.model_cache + ". Or move it to `model`.");
        }
