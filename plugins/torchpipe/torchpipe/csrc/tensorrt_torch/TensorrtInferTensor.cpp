...
namespace torchpipe
{
        ...
        const hami::dict &dict_config)
    {
        ...
        if (instance_num_ > 1)
        {
            HAMI_ASSERT(!torch_is_using_default_stream(),
        ...
        const std::vector<hami::dict> &input_output)
    {
        ...
        // hami::helper::ScopedTimer timer("tensorrt infer. size = " + std::to_string(inputs[0].sizes()[0]), 0.01);
        ...
        if (input_output[0]->find("outputs") != input_output[0]->end())
            outputs = hami::dict_gets<torch::Tensor>(input_output[0], "outputs");
