...
plt.rcParams.update({
    ...
    'axes.titlesize': 18,      # 增大坐标轴标题大小
    'axes.labelsize': 17,      # 增大坐标轴标签大小
    'xtick.labelsize': 15,     # 增大x轴刻度标签大小
    'ytick.labelsize': 15,     # 增大y轴刻度标签大小
    'legend.fontsize': 16,     # 增大图例字体大小
    'figure.titlesize': 20     # 增大图形标题大小
})
...
def plot_comparison(data):
    ...
    # 调整子图间距，确保不重叠
    plt.subplots_adjust(wspace=0.3, hspace=0.35, left=0.1,
    ...
    for i, id_name in enumerate(ids):
        ...
        if x_vals:
            line, = ax1.plot(x_vals, y_vals, marker=markers[marker_idx],
                             ...
                             markersize=8, linewidth=2.5, markeredgewidth=1.5)  # 增大标记和线宽
            # 为每个算法添加图例句柄
            legend_handles.append(line)

    ax1.set_xlabel('Rate Scale (req/s, CV=0.5, SLO=7)', fontsize=17, labelpad=10)
    ax1.set_ylabel('Goodput (req/s)', fontsize=17, labelpad=10)
    ...
    ax2.set_xlabel('CV Scale (Rate=3, SLO=7)', fontsize=17, labelpad=10)
    ...
    ax3.set_xlabel('Rate Scale (req/s, CV=0.5, SLO=3)', fontsize=17, labelpad=10)
    ax3.set_ylabel('Goodput (req/s)', fontsize=17, labelpad=10)
    ...
    ax4.set_xlabel('SLO Scale (Rate=3, CV=0.5)', fontsize=17, labelpad=10)
    ax4.set_xticks(available_slos)
    ax4.tick_params(axis='both', which='major', labelsize=15)
    ax4.grid(True, linestyle='--', alpha=0.7)

    # 设置所有y轴的范围大致相等
    y_min = min(all_y_values) * 0.95  # 留出10%的边距
    y_max = max(all_y_values) * 1.05  # 留出10%的边距
