...
Tensor _EmptyCPUInt32(int64_t n) {
...
Array<Tensor> ScheduleStepFunc(
  ...
  Tensor out_ids = _EmptyCPUInt32(int64_t(sel_ids.size()));
  Tensor out_new_tokens = _EmptyCPUInt32(int64_t(sel_new_tokens.size()));
  Tensor out_is_prefill = _EmptyCPUInt32(int64_t(sel_is_prefill.size()));
  Tensor out_should_output = _EmptyCPUInt32(int64_t(sel_should_output.size()));
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("custom.schedule_step", custom::ScheduleStepFunc);
}
